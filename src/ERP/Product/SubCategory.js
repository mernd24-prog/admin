...
const SubCategoryPage = () => {
  ...
  const tableHeadings = ["#", "Sub Category", "Status", "Actions"];
  ...
  const handleAddNavigate = () => {
    console.log("Navigate to add new sub category form");
  };
  ...
  return (
    <div className="p-4 sm:p-6 mx-auto max-w-7xl overflow-auto">
      ...
      <div className="flex flex-row justify-between items-center gap-2 mb-4">
        <div>
          <h1 className="text-xl md:text-2xl font-semibold">Sub Category</h1>
          <h3 className="text-sm text-gray-500 font-medium">
            <span >Dashboard</span> / Product /{" "}
            <span className="text-[#181c32]">Sub Category</span>
          </h3>
        </div>
        <div className="">
          <AddButton onClick={handleAddNavigate} text="Add New Sub Category" />
        </div>
      </div>
