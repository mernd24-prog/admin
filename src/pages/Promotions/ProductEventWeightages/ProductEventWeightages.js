...
const ProductEventWeightages = () => {
  ...
  const fetchData = useCallback(async () => {
    try {
      await dispatch(getContentPages({ page: pageNo, limit: PAGE_SIZE, q: filters.search || undefined, pageType: 'product_event_weightage' })).unwrap();
    } catch (err) {
    ...
    <>
      ...
      <div className='p-6 overflow-hidden overflow-x-auto overflow-y-auto max-w-7xl mx-auto space-y-3'>
        <h3 className='text-gray-500 text-sm font-semibold py-3'>
          <Link to='/app/home'>Home</Link> / <span className='text-[#181c32]'>Product Event Weightages</span>
        </h3>
        <div className='p-4 overflow-auto bg-white rounded-lg border border-[#E6E6E6]'>
          <SearchComponent tableHeadings={['Event', 'Weightage']} data={tableRows} selectedRow={selectedRow} setSelectedRow={setSelectedRow} loading={selector.loading} filters={filters} setFilters={setFilters} isSearchShow={true} isActionButton={false} isStatusAction={false} isDelete={false} applyFilters={() => { setPageNo(1); fetchData(); }} handleSearchRemove={() => { setFilters({ search: '' }); setPageNo(1); }} />
          <TableData Heading='Product Event Weightages' tableHeadings={['Event', 'Weightage']} data={tableRows} showSearch={true} showFilter={false} showSummary={false} showAddButton={false} isHeaderCheckbox={false} totalData={total} />
        </div>
