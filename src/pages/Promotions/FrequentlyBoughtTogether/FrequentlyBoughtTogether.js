...
const FrequentlyBoughtTogether = () => {
  ...
  const fetchProducts = useCallback(async () => {
    try {
      ...
      toast.error(err?.message || err || 'Failed to fetch frequently bought together products');
    } finally {
      setIsLoading(false);
    }
  ...
  const tableRows = list.map((product) => {
    ...
    const buyTogether = Array.isArray(product?.crossSellProducts) ? product.crossSellProducts : [];
    ...
    return [
      ...
      <div className='flex flex-wrap gap-1 max-w-[700px]'>
        {buyTogether.length > 0 ? buyTogether.map((value) => (
    ...
    <>
      ...
      <div className='p-6 overflow-hidden overflow-x-auto overflow-y-auto max-w-7xl mx-auto space-y-3'>
        <h3 className='text-gray-500 text-sm font-semibold py-3'>
          <Link to='/app/home'>Home</Link> / <span className='text-[#181c32]'>Frequently Bought Together</span>
        </h3>
      ...
      <AddEditFBT
        ...
        onSubmit={async (data) => {
          try {
            ...
            toast.error(error?.message || error || 'Failed to update frequently bought together');
          }
        }}
      />
    </>
