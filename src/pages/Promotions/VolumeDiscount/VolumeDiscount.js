...
const VolumeDiscount = () => {
  ...
  const fetchProducts = useCallback(async () => {
    try {
      ...
      toast.error(err?.message || err || 'Failed to fetch volume discount products');
    } finally {
      setIsLoading(false);
    }
  ...
  const tableHeadings = ['Product Name', 'Minimum Purchase Quantity', 'Discount(%)', 'Action'];
  ...
  const tableRows = list.map((product) => {
    ...
    const minQty = Number(firstDefined(product?.metadata?.volumeDiscount?.minimumQuantity, product?.minPurchaseQuantity, product?.minimumPurchaseQty, 1));
    const discount = Number(firstDefined(product?.metadata?.volumeDiscount?.discount, product?.volumeDiscount, product?.bulkDiscountPercent, 0));
    ...
    <>
      ...
      <div className='p-6 overflow-hidden overflow-x-auto overflow-y-auto max-w-7xl mx-auto space-y-3'>
        <h3 className='text-gray-500 text-sm font-semibold py-3'>
          <Link to='/app/home'>Home</Link> / <span className='text-[#181c32]'>Volume Discount</span>
        </h3>
      ...
      <AddEditVolumeDiscount
        ...
        onSubmit={async (formData) => {
          ...
          try {
            ...
            toast.error(err?.message || err || 'Failed to update volume discount');
          }
        }}
      />
    </>
