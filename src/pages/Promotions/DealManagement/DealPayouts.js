...
const DealPayouts = () => {
  ...
  const fetchPayouts = useCallback(async () => {
    try {
      ...
      await dispatch(getDealPayouts({ ...params, offset: (params.page - 1) * params.limit })).unwrap();
    } catch (err) {
  ...
  const handleGenerate = useCallback(async () => {
    if (!generateForm.fromDate || !generateForm.toDate) { toast.error("Date range required"); return; }
    try {
      setGenerating(true);
      await dispatch(generateDealPayout({
  ...
  const handleProcess = useCallback(async () => {
    ...
    try {
      setProcessing(true);
      await dispatch(processDealPayout({
      ...
      setProcessConfirm({ open: false, payout: null, status: "paid", referenceId: "", note: "" });
      fetchPayouts();
    } catch (err) {
  ...
  const COLUMNS = [
    ...
    { key: "createdAt", label: "Generated", render: (v) => <span className="text-xs text-gray-500">{fmt(v)}</span> },
    {
      key: "_actions",
      label: "Actions",
      render: (_, row) => (
        <div className="flex gap-1">
          ...
          <PermissionGuard module="deals" action={ACTIONS.APPROVE} hide>
            {row.status === "generated" && (
              <button
                onClick={() => setProcessConfirm({ open: true, payout: row, status: "paid", referenceId: "", note: "" })}
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <DefaultModal isOpen={showGenerate} onClose={() => setShowGenerate(false)} title="Generate Deal Payouts">
      ...
      <ConfirmModal
        ...
        onCancel={() => setProcessConfirm({ open: false, payout: null, status: "paid", referenceId: "", note: "" })}
        loading={processing}
        ...
        <div className="mt-3 space-y-3">
          <div>
            ...
            <select
              ...
              onChange={(e) => setProcessConfirm((p) => ({ ...p, status: e.target.value }))}
            >
          ...
          <Input label="Reference ID" value={processConfirm.referenceId} onChange={(e) => setProcessConfirm((p) => ({ ...p, referenceId: e.target.value }))} placeholder="Bank / transfer reference..." />
          <Input label="Note" value={processConfirm.note} onChange={(e) => setProcessConfirm((p) => ({ ...p, note: e.target.value }))} placeholder="Optional note..." />
        </div>
      </ConfirmModal>
    </div>
  );
