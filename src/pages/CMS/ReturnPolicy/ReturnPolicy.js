...
import { roleBasedAccess } from '../../../_helpers/globalFunctions';
...
const ReturnPolicy = () => {
    ...
    const size = 10;
    const accessOptions = roleBasedAccess();
    ...
    useEffect(() => {
        const reqData = {
            ...
            searchFields: 'name',
            select: 'name isDisable role_id content',
            query: JSON.stringify({ type: "return-policy" })
        };
    ...
    const validateAddUserForm = () => {
        ...
        if (!formData?.store_id) {
            newErrors.store_id = 'User is required';
            isValid = false;
        }
    ...
    const handleAddUserSubmit = (e) => {
        ...
        dispatch(createCategory(reqData))
            ...
            .then((res) => {
                ...
                toast.success(res.message || "Payment Policy created successfully");
                handleClose();
                setIsRefresh(!isRefresh);
            })
            .catch((error) => {
                console.error("Error creating Payment Policy:", error);
                toast.error(error || "Error in creating Payment Policy");
            });
    };
    ...
    const handleEditUserSubmit = (e) => {
        ...
        dispatch(updateCMSCategory(reqData))
            ...
            .then((res) => {
                ...
                toast.success(res.message || "Payment Policy updated successfully");
                handleEditClose();
                setIsRefresh(!isRefresh);
            })
            .catch((error) => {
                console.error("Error updating Payment Policy :", error);
                toast.error(error || "Error in updating Payment Policy ");
            });
    };
    ...
    const confirmDelete = () => {
        dispatch(deleteCategory(deleteData))
            ...
            .then((res) => {
                ...
                toast.success(res.message || "Payment Policy  deleted successfully");
                setShowDeleteConfirmation(false);
                setIsRefresh(!isRefresh);
            })
            .catch((error) => {
                console.error("Error deleting Payment Policy :", error);
                toast.error(error || "Error in deleting Payment Policy ");
            });
    };
    ...
    const handleEdit = (user) => {
        ...
        const userOption = accessOptions.find(option => option.value === user.role_id);
    ...
    const tableRows = getListData?.map((user) => [
        ...
        <span key={`actions-${user._id}`}>
            <ActionButtons
                ...
                onListing={() => {
                    navigate(`/app/return-policy-list/${user._id}`);
                }}
    ...
    const handleContentChangeSimple = useCallback((value) => {
        setForm(prev => {
            ...
            <div className='max-w-7xl mx-auto'>
                <div className='overflow-hidden overflow-y-auto py-6'>
                    <div className="flex justify-between items-center">
                        <h3><Link to="/app/home">Home</Link> / <b>Return Policy</b></h3>
                        <AddButton
                ...
                >
                    ...
                    <div className="px-4 pt-2">
                        <TextEditor
                            ...
                            onChange={handleContentChangeSimple}
                            placeholder="Enter privacy policy content..."
                            height="300px"
                            error={errors.content}
                            required
                ...
                <DeletePopup
                    ...
                    DeleteHeading={'Are you sure you want to delete this Privacy Policy?'}
                />
                ...
                <StatusPopup
                    ...
                    heading={`Are you sure you want to ${toggleStates?.isDisable ? 'enable' : 'disable'} this Privacy Policy?`}
                />
            </div>
        </>
    );
