...
// Redux actions
import { create, enableDisable, getList, softDelete, update } from '../../../Redux/productSlice';
...
const ProductCategories = () => {
  ...
  const handleToggle = useCallback((data) => () => {
    const updateChildrenStatus = (categories, categoryId, status) => {
      return categories.map(category => {
        ...
        if (category.subCategories?.length > 0) {
          return {
            ...category,
            subCategories: updateChildrenStatus(category.subCategories, categoryId, status)
          };
        }
        return category;
      });
    ...
    dispatch(enableDisable({
      ...
      .then((res) => {
        ...
        setCategories(prevCategories => updateChildrenStatus(prevCategories, data._id, newStatus));
    ...
    if (filters.search) {
      ...
      const filtered = allCategories.map(category => {
        ...
        const filteredSubCategories = category.subCategories?.map(subCategory => {
          ...
          const filteredChildren = subCategory.subCategories?.filter(child =>
  ...
  const renderCategory = (category, level = 0, parentId = null, allCategories = []) => {
    const hasChildren = category.subCategories?.length > 0;
    const marginLeft = level > 0 ? `${level * 40}px` : '0';

    const isLastChild = parentId ? (() => {
      const parent = findCategoryById(allCategories, parentId);
      return parent?.subCategories?.indexOf(category) === parent?.subCategories?.length - 1;
    })() : false;
    ...
    return (
      <div key={category._id} className="relative">
        ...
        {/* Category Row */}
        <div
          className={`flex items-center justify-between p-3 mb-1 ${level === 0
        ...
        >
          <div className="flex items-center gap-3">
            {hasChildren && (
              <button
                onClick={() => toggleExpand(category._id, level, parentId)}
                className={`flex items-center justify-center w-5 h-5 rounded-sm border ${category.isExpanded
                  ? 'bg-gray-600 border-gray-600 text-black'
                  : 'bg-white border-gray-400 text-gray-600 hover:border-gray-600'
                  }`}
              >
                {category.isExpanded ? <FaMinus size={10} /> : <FaPlus size={10} />}
              </button>
            )}

            <span className={`capitalize ${level === 0
        ...
        {category.isExpanded && hasChildren && (
          <div className="relative">
            {category.subCategories.map((subCategory, index) =>
              renderCategory(subCategory, level + 1, category._id, allCategories)
            )}
          </div>
        )}
      </div>
      ...
      <div className="mx-auto px-4 py-6">
        <div className="max-w-7xl mx-auto space-y-6">
          <div className="flex justify-between items-center">
            <div>
              <h3>Home / <b>Product Categories</b></h3>
            </div>
            <AddButton
              className=" "
              onClick={() => {
