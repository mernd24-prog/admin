...
const Store = () => {
  ...
  const handleAction = (type, shop = null) => {
    if (type === 'EDIT' && shop) {
      ...
      if (shop.country_code) {
        dispatch(getAllStateList({ query: JSON.stringify({ country_code: shop.country_code }) }));
      }
      if (shop.state_code) {
        dispatch(getAllCityList({ query: JSON.stringify({ state_code: shop.state_code }) }));
      }
      if (shop.city_code) {
        dispatch(getAllZipCodeList({ query: JSON.stringify({ city_code: shop.city_code }) }));
      }
    } else {
  ...
  const validateStoreForm = (formValues) => {
    ...
    if (!formValues.contact_person?.trim()) errors.contact_person = "Contact person is required";
    if (!formValues.address?.trim()) errors.address = "Address is required";
    if (!formValues.businessLicense?.trim()) errors.businessLicense = "Business license is required";
    if (!formValues.gstNumber?.trim()) errors.gstNumber = "GST number is required";
    if (!formValues.panNumber?.trim()) errors.panNumber = "PAN number is required";
    ...
    if (modalState.type !== "EDIT") {
      ...
      if (!formValues.confirmPassword?.trim()) errors.confirmPassword = "Confirm Password  is required";
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (modalState.type === 'EDIT') {
        ...
        const { password, confirmPassword, ...cleanedPayload } = payloadWithTime;
  ...
  const handleToggleAction = async (data) => {
    ...
    try {
      ...
      if (response.message) {
        toast.success(`Store ${apiPayload.isDisable ? 'disabled' : 'enabled'} successfully.`);
        fetchStoreList();
      } else {
  ...
  const tableRows = apiRes?.list?.map((shop, index) => {
    ...
    row.push(
      <p key={`address-${index}`} className='truncate text-wrap break-words w-52'>{shop?.address || '-'}</p>,
      <div key={`contact-${index}`}>
        <p><span>{shop?.mobile || '-'}</span></p>
        <p><span>{shop?.email || '-'}</span></p>
      </div>,
      ...
      <div key={`actions-${index}`}>
        <ActionButtons
          ...
          onPasswordChange={() => { setIsPasswordUpdateModal(true); setFormValues({ user_id: shop?._id }) }}
  ...
  const handleClose = () => {
    setIsPasswordUpdateModal(false); setFormValues(INITIAL_FORM_STATE)
    setFormErrors({})
  }
  ...
  return (
    <>
      ...
      <div className='overflow-hidden overflow-x-auto overflow-y-auto max-w-7xl mx-auto space-y-3 py-10'>
        ...
        <div className='p-4 space-y-4 py-6 text-xs'>
          ...
          <div>
            <div className=' grid grid-cols-1 md:grid-cols-2 gap-2'>
              ...
              <Input
                labelName="Email *" name="email" type="email" value={formValues.email} onChange={handleInputChange} error={formErrors.email} placeholder="Enter email address"
              />
            </div>
          ...
          <Input labelName={`Longitude's`} value={formValues?.location?.coordinates[0]} placeholder={`ex:28.69564`} name={`coordinates`} onChange={(e) => handleInputChange(e, 0)}
          />
          <Input labelName={`Latitudes`} value={formValues?.location?.coordinates[1]} name={`coordinates`} placeholder={`ex:28.69564`} onChange={(e) => handleInputChange(e, 1)}
          />
          <span className='text-red-500 text-xs col-span-2'>{formErrors.coordinates}</span>
      ...
      <DefaultMiddleModal isOpen={isPasswordUpdateModal} onClose={handleClose} title={`Update your store password`} onSubmit={handleSubmitUpdatePassword}>
        <div className='p-2'>
          <Input type='password' value={formValues?.password} onChange={handleInputChange} name="password" labelName={`Password`} />
          <Input type='password' value={formValues?.confirmPassword} onChange={handleInputChange} name="confirmPassword" labelName={`Confirm Password`} />
