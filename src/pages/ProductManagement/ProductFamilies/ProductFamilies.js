...
const ProductFamilies = () => {
  ...
  const productList = productSelector?.getAllProductsData?.data?.data?.list || [];
  ...
  const categoryOptions = useMemo(() => {
    ...
    const addOptions = (categories, prefix = '') => {
      ...
      categories.forEach((category) => {
        ...
        options.push({ value: category.categoryKey || category._id || category.id, label });
  ...
  const productBrandOptions = useMemo(() => {
    ...
    return [...brands].sort((a, b) => a.localeCompare(b)).map((brand) => ({ value: brand, label: brand }));
  }, [productList]);

  const categoryLabelMap = useMemo(
    () => Object.fromEntries(categoryOptions.map((item) => [String(item.value), item.label])),
    [categoryOptions],
  );
  const sellerLabelMap = useMemo(
    () => Object.fromEntries(sellerOptions.map((item) => [String(item.value), item.label])),
    [sellerOptions],
  );

  const fetchList = useCallback(() => {
    dispatch(getProductFamilies({ page: pageNo, limit: PAGE_SIZE, category: filters.search }));
  ...
  const handleBulkAction = async (action) => {
    if (!selectedRow.length) return toast.warning('Please select at least one family');
  ...
  const rows = list.map((item) => [
    <CustomCheckbox
      ...
      onChange={(event) =>
        setSelectedRow((prev) =>
          event.target.checked ? [...prev, idFromRecord(item)] : prev.filter((id) => id !== idFromRecord(item)),
        )
      }
    />,
    <span className="font-mono text-xs">{item.familyCode || item.code}</span>,
    <span>{item.title || item.name}</span>,
    <span>{categoryLabelMap[String(item.category || '')] || item.category || '-'}</span>,
    <span>{sellerLabelMap[String(item.sellerId || '')] || item.sellerName || item.sellerId || '-'}</span>,
    ...
    <ActionButtons
      onEdit={() => {
        setFormData({
          ...
          variantAxes: Array.isArray(item.variantAxes) && item.variantAxes.length ? item.variantAxes : [''],
  ...
  const allSelected = useMemo(() => selectedRow.length === list.length && list.length > 0, [selectedRow.length, list.length]);
  ...
  return (
    <>
          ...
          <div className='overflow-y-auto bg-white rounded-lg border border-[#E6E6E6]'>
            <div className="bg-white p-2 border-b mb-4">
              <SearchComponent
                ...
                applyFilters={() => { setPageNo(1); setIsRefresh((v) => !v); }}
                handleSearchRemove={() => { setFilters({ search: '' }); setPageNo(1); setIsRefresh((v) => !v); }}
              />
            </div>
            <TableData
              ...
              tableHeadings={[
                <input
                  ...
                  checked={allSelected}
                  onChange={(event) => setSelectedRow(event.target.checked ? list.map((item) => idFromRecord(item)) : [])}
                />,
                'Family Code',
                'Title',
                'Category',
      ...
      <DefaultModal isOpen={isModalOpen} onClose={closeModal} title={formData._editingId ? 'Edit Product Family' : 'Add Product Family'} size="lg">
        <form onSubmit={handleSubmit} className="space-y-4">
          <FormInput label="Family Code" name="familyCode" value={formData.familyCode} onChange={(e) => setFormData((p) => ({ ...p, familyCode: e.target.value }))} error={errors.familyCode} disabled={Boolean(formData._editingId)} />
          <FormInput label="Title" name="title" value={formData.title} onChange={(e) => setFormData((p) => ({ ...p, title: e.target.value }))} error={errors.title} />
          <FilterSelect
            label="Category"
            options={categoryOptions}
            value={categoryOptions.find((opt) => String(opt.value) === String(formData.category || '')) || null}
            onChange={(option) => { setFormData((prev) => ({ ...prev, category: option?.value || '' })); setErrors((prev) => ({ ...prev, category: undefined })); }}
          ...
          <FilterSelect
            ...
            onChange={(option) => setFormData((prev) => ({ ...prev, sellerId: option?.value || '' }))}
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              ...
              <button type="button" className="text-xs text-blue-600" onClick={() => setFormData((p) => ({ ...p, variantAxes: [...(p.variantAxes || []), ''] }))}>Add Axis</button>
            </div>
            {(formData.variantAxes || []).map((axis, idx) => (
              <div key={`axis-${idx}`} className="flex gap-2">
                <input className="w-full border border-gray-300 rounded-md px-3 py-2 text-sm" value={axis} onChange={(e) => setFormData((p) => ({ ...p, variantAxes: (p.variantAxes || []).map((v, i) => (i === idx ? e.target.value : v)) }))} placeholder="e.g. color" />
                <button type="button" className="px-2 text-red-600" onClick={() => setFormData((p) => ({ ...p, variantAxes: (p.variantAxes || []).filter((_, i) => i !== idx) || [''] }))}>X</button>
              </div>
            ))}
          </div>
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              ...
              <button type="button" className="text-xs text-blue-600" onClick={() => setFormData((p) => ({ ...p, baseAttributes: [...(p.baseAttributes || []), emptyAttribute] }))}>Add Attribute</button>
            </div>
            {(formData.baseAttributes || []).map((row, idx) => (
              <div key={`attr-${idx}`} className="grid grid-cols-2 gap-2">
                <input className="border border-gray-300 rounded-md px-3 py-2 text-sm" placeholder="key" value={row.key} onChange={(e) => setFormData((p) => ({ ...p, baseAttributes: (p.baseAttributes || []).map((r, i) => (i === idx ? { ...r, key: e.target.value } : r)) }))} />
                <div className="flex gap-2">
                  <input className="w-full border border-gray-300 rounded-md px-3 py-2 text-sm" placeholder="value" value={row.value} onChange={(e) => setFormData((p) => ({ ...p, baseAttributes: (p.baseAttributes || []).map((r, i) => (i === idx ? { ...r, value: e.target.value } : r)) }))} />
                  <button type="button" className="px-2 text-red-600" onClick={() => setFormData((p) => ({ ...p, baseAttributes: (p.baseAttributes || []).filter((_, i) => i !== idx) || [emptyAttribute] }))}>X</button>
                </div>
              </div>
            ))}
          </div>
      ...
      {deleteTarget && (
        <DeletePopup Heading='Delete Product Family' Body='Are you sure you want to delete this product family?' onCancel={() => setDeleteTarget(null)} onDelete={handleDelete} />
      )}

      {toggleTarget && (
        <StatusPopup title={toggleTarget.status === 'active' ? 'Deactivate Product Family' : 'Activate Product Family'} message={`Are you sure you want to ${toggleTarget.status === 'active' ? 'deactivate' : 'activate'} this product family?`} onConfirm={handleToggle} onCancel={() => setToggleTarget(null)} />
      )}
    </>
  );
};
