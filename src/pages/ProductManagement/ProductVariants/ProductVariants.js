...
const ProductVariants = () => {
  ...
  const familyOptions = useMemo(
    () =>
      families.map((family) => ({
        value: family.familyCode || family.code,
        label: `${family.title || family.name || family.familyCode} (${family.familyCode || family.code})`,
      })),
    [families],
  );
  ...
  const productOptions = useMemo(
    () =>
      products.map((product) => ({
        ...
        label: `${product.title || product.name || product.slug || product._id}`,
      })),
    [products],
  );
  ...
  const familyLabelMap = useMemo(
    () =>
      Object.fromEntries(
        familyOptions.map((item) => [String(item.value), item.label]),
      ),
    [familyOptions],
  );
  const productLabelMap = useMemo(
    () =>
      Object.fromEntries(
        productOptions.map((item) => [String(item.value), item.label]),
      ),
    [productOptions],
  );
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      toast.error(
        error?.message || error || "Failed to delete product variant",
      );
    }
  };
  ...
  const handleBulkAction = async (action) => {
    if (!selectedRow.length)
      return toast.warning("Please select at least one variant");
    const status =
      action === "Active"
        ? "active"
        : action === "Inactive"
    ...
    try {
      await Promise.all(
        selectedRow.map((variantId) =>
          dispatch(updateProductVariant({ variantId, status })).unwrap(),
        ),
      );
      ...
      toast.error(
        error?.message || error || "Failed to update selected variants",
      );
    }
  };
  ...
  const tableRows = variants.map((variant) => [
    ...
    <span>
      {familyLabelMap[String(variant.familyCode || "")] ||
        variant.familyCode ||
        "-"}
    </span>,
    <span>
      {productLabelMap[String(variant.productId || "")] ||
        variant.productTitle ||
        variant.productId ||
        "-"}
    </span>,
    <span>
      {sellerLabelMap[String(variant.sellerId || "")] ||
        variant.sellerName ||
        variant.sellerId ||
        "-"}
    </span>,
    ...
    <ToggleButton
      isToggle={variant.status === "active"}
      handleClick={() => setToggleTarget(variant)}
    />,
    <ActionButtons
          ...
          <div className="overflow-y-auto bg-white rounded-lg border border-[#E6E6E6]">
            ...
            <TableData
              ...
              handleHeaderCheckboxChange={(event) =>
                setSelectedRow(
                  event.target.checked
                    ? variants.map((item) => variantIdFromRecord(item))
                    : [],
                )
              }
        ...
        <div className="p-4 grid grid-cols-1 md:grid-cols-2 gap-4">
          <FilterSelect
            ...
            value={
              familyOptions.find(
                (opt) =>
                  String(opt.value) === String(formData.familyCode || ""),
              ) || null
            }
            onChange={(option) => {
              setFormData((prev) => ({
                ...prev,
                familyCode: option?.value || "",
              }));
              setErrors((prev) => ({ ...prev, familyCode: undefined }));
            }}
          ...
          <FilterSelect
            ...
            value={
              productOptions.find(
                (opt) => String(opt.value) === String(formData.productId || ""),
              ) || null
            }
            onChange={(option) => {
              setFormData((prev) => ({
          ...
          <FormInput
            ...
            onChange={(e) =>
              setFormData((p) => ({ ...p, stock: Number(e.target.value || 0) }))
            }
            error={errors.stock}
            required
          />
          ...
          <FormInput
            ...
            options={productStatuses.options}
            disabled={productStatuses.loading}
          />
        </div>
        ...
        <div className="p-4 space-y-2">
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              onClick={() =>
                setFormData((p) => ({
                  ...p,
                  attributes: [...(p.attributes || []), { ...emptyAttribute }],
                }))
              }
            >
              Add Attribute
          ...
          {!(formData.attributes || []).length && (
            <p className="text-xs text-gray-500">
              Optional. Add only if this variant needs custom attribute
              overrides.
            </p>
          )}
            ...
            <div key={`attr-${idx}`} className="grid grid-cols-2 gap-2">
              <input
                ...
                onChange={(e) =>
                  setFormData((p) => ({
                    ...p,
                    attributes: (p.attributes || []).map((r, i) =>
              ...
              <div className="flex gap-2">
                ...
                <button
                  ...
                  onClick={() =>
                    setFormData((p) => ({
                      ...p,
                      attributes: (p.attributes || []).filter(
                        (_, i) => i !== idx,
                      ),
                    }))
                  }
