...
const PackageFormModal = ({ open, form, errors, submitting, onClose, onChange, onSubmit }) => {
  if (!open) return null;
    ...
    <div className="fixed inset-0 z-[9998] flex items-center justify-center">
      ...
      <div className="relative w-full max-w-2xl max-h-[92vh] overflow-y-auto bg-white rounded-lg shadow-xl mx-4">
        ...
        <form onSubmit={onSubmit} className="p-5 grid grid-cols-1 md:grid-cols-2 gap-4">
          <Input labelName="Package Name" name="name" value={form.name} onChange={onChange} error={errors.name} required />
          <Input labelName="Code" name="code" value={form.code} onChange={onChange} error={errors.code} placeholder="Optional" />
          <Input labelName="Length" type="number" name="length" value={form.length} onChange={onChange} error={errors.length} min={0} required />
          <Input labelName="Width" type="number" name="width" value={form.width} onChange={onChange} error={errors.width} min={0} required />
          <Input labelName="Height" type="number" name="height" value={form.height} onChange={onChange} error={errors.height} min={0} required />
          <Input labelName="Unit" type="select" name="unit" value={form.unit} onChange={(option) => onChange({ target: { name: 'unit', value: option?.value || 'cm' } })} options={UNIT_OPTIONS} error={errors.unit} required />
          <Input labelName="Max Weight" type="number" name="maxWeight" value={form.maxWeight} onChange={onChange} error={errors.maxWeight} min={0} />
...
const ShippingPackages = () => {
  ...
  const columns = [
    ...
    { key: 'maxWeight', label: 'Max Weight', render: (value) => (value ? `${value} kg` : '—') },
      ...
      render: (_, row) => (
        <div className="flex items-center gap-2">
          <PermissionGuard module="delivery" action={ACTIONS.EDIT}>
            <button type="button" onClick={() => openEdit(row)} className="inline-flex items-center justify-center w-8 h-8 rounded-lg border border-gray-200 text-gray-600 hover:bg-gray-50" title="Edit shipping package">
              <MdEdit size={16} />
            </button>
          </PermissionGuard>
          <PermissionGuard module="delivery" action={ACTIONS.DELETE}>
            <button type="button" onClick={() => setDeleteTarget(row)} className="inline-flex items-center justify-center w-8 h-8 rounded-lg border border-red-100 text-red-500 hover:bg-red-50" title="Delete shipping package">
              <MdDelete size={16} />
            </button>
          </PermissionGuard>
        </div>
  ...
  return (
    <div className="p-6">
      <PageHeader
        ...
        breadcrumbs={[{ label: 'Tax & Compliance' }, { label: 'Shipping Rules' }]}
        actions={
      ...
      <DataTable
        ...
        exportConfig={{ filename: 'shipping-packages', columns }}
      />
