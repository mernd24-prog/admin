...
const InventoryAdjustment = () => {
  ...
  const searchProducts = async () => {
    ...
    setSearching(true);
    try {
      const res = await axiosProvider.get('/products', { params: { search, limit: 10 } });
  ...
  const handleSubmit = async () => {
    if (!selected || !qty || !reason) return toast.warn('Fill all required fields');
    setSubmitting(true);
    try {
      ...
      setStep(1); setSelected(null); setQty(''); setReason(''); setNote('');
    } catch {
  ...
  return (
    <div className="p-6">
      <PageHeader
        ...
        breadcrumbs={[{ label: 'Inventory Management' }, { label: 'Inventory Adjustment' }]}
      />

      <PermissionGuard module="inventory" action={ACTIONS.ADJUST} fallback={
        ...
        <div className="max-w-2xl">
          ...
          <FormSection title="1. Find Product" icon={<MdSearch size={16} />} required>
                ...
                {results.map((p) => (
                  ...
                  >
                    <div>
                      ...
                      <div className="text-xs text-gray-400">{p.sku || 'No SKU'} · Stock: {p.stock ?? 0}</div>
                    </div>
                    {selected?._id === p._id && <MdCheck size={16} className="text-[#989AFF]" />}
                  </div>
                ))}
              </div>
            )}
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <div className="text-xs text-gray-400 mt-0.5">
                      SKU: {selected.sku || '—'} · Current Stock: <strong>{selected.stock ?? 0}</strong>
                    </div>
                  </div>
          ...
          {selected && (
            <FormSection title="2. Adjustment Details" icon={<MdTune size={16} />} required className="mt-4">
              {/* Type */}
              <div className="grid grid-cols-3 gap-3 mb-4">
                {TYPES.map((t) => {
                  ...
                  return (
                    ...
                    >
                      <Icon size={20} className={adjustType === t.value ? 'text-[#989AFF]' : t.color} />
                      {t.label}
                    </button>
                  );
                })}
