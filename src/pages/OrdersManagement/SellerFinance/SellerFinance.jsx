...
const payoutDecision = (row = {}) => {
  ...
  if (
    row.payout_id ||
    row.payoutId ||
    ["released", "paid", "completed"].includes(status)
  ) {
    return {
      allowed: false,
      label: "Already in payout",
      reason:
        "This item was already added to a payout/settlement. It is shown for history only.",
    };
  }
...
const loadFinance = useCallback(async () => {
  ...
  const orgOpts = useMemo(
    () => [
      {
        value: "",
        label: filters.sellerId ? "All Organizations" : "Select seller first",
      },
      ...organizationOptions,
    ],
  ...
  const actionableCommissions = useMemo(
    () =>
      visibleCommissions.filter((row) => {
        ...
        if (
          !filters.status &&
          (hasPayout || ["released", "paid", "completed"].includes(status))
        )
          return false;
        return true;
      }),
