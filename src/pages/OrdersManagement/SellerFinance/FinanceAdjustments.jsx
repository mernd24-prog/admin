...
import { MdArrowForward, MdRefresh, MdVisibility } from "react-icons/md";
...
import { FinanceEmptyState, FinanceMetricCard, FinanceNav, FinanceStatusBadge, financeDateTime, financeMoney, signedFinanceMoney, unwrapFinance } from "./financeUi";
...
export default function FinanceAdjustments() {
  ...
  return <div className="space-y-5"><PageHeader title="Adjustments" subtitle="Understand why your earnings changed." breadcrumbs={[{ label: "My Finance & Payouts" }, { label: "Adjustments" }]} actions={<button type="button" className="admin-btn-secondary" onClick={load}><MdRefresh /> Refresh</button>} /><FinanceNav />
    ...
    <div className="flex gap-2 overflow-x-auto">{CATEGORIES.map(([key, label]) => <button key={label} type="button" onClick={() => setParams(key ? { type: key } : {})} className={`whitespace-nowrap rounded-full border px-4 py-2 text-sm font-semibold ${category === key ? "border-[var(--admin-navy)] bg-[var(--admin-navy)] text-white" : "border-[var(--admin-line)] bg-white"}`}>{label}</button>)}</div>
