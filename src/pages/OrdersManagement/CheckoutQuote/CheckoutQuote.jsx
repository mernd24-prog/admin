...
const BuyerSearchField = ({ value, onChange }) => {
  ...
  return (
    <div ref={ref} className="relative">
      <div
        className="admin-input flex items-center cursor-pointer min-h-[38px]"
        onClick={() => setOpen((o) => !o)}
      >
        {value && label ? (
...
const CheckoutQuote = () => {
  ...
  const sellerSettlements = Array.isArray(quote?.sellerSettlements)
    ? quote.sellerSettlements
    : [];
  ...
  const submitQuote = async () => {
    ...
    try {
      const response = await axiosProvider.post(
        ENDPOINTS.orders.checkoutQuote,
        {
      ...
      toast.error(
        err?.response?.data?.message || "Unable to calculate checkout quote",
      );
    } finally {
      setLoading(false);
    }
      ...
      <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_380px]">
        <div className="admin-card space-y-5 p-5">
          ...
          <label className="block space-y-1">
            ...
            <input
              className="admin-input max-w-sm uppercase"
              value={couponCode}
              onChange={(event) =>
          ...
          <section className="space-y-3">
            ...
            <div className="space-y-3">
              {items.map((item, index) => (
                <div
                  key={index}
                  className="grid gap-3 rounded border p-3 md:grid-cols-[minmax(0,1fr)_minmax(0,1fr)_minmax(0,1fr)_90px_36px]"
                >
                  <input
                    className="admin-input"
                  ...
                  <input
                    ...
                    placeholder="Variant ObjectId"
                  />
                  <input
                    className="admin-input"
                    value={item.variantSku}
                    onChange={(event) =>
                      updateItem(index, "variantSku", event.target.value)
                    }
                  ...
                  <button
                    type="button"
                    className="rounded border text-red-600 disabled:opacity-40"
        ...
        <aside className="admin-card h-fit p-5">
          <div className="mb-4 flex items-center justify-between">
            ...
            {quote && (
              <StatusBadge
                status={quoteTotals.paymentProvider || "quoted"}
                dot
              />
            )}
          </div>
          {!quote ? (
            <p className="text-sm text-gray-500">
              Enter checkout details and calculate a quote.
            </p>
          ) : (
            <div className="space-y-5">
              <div className="space-y-2 text-sm">
                <div className="flex justify-between">
                  <span>Items</span>
                  <span>{money(summary.itemAmount)}</span>
                </div>
                <div className="flex justify-between">
                  <span>Discount</span>
                ...
                <div className="flex justify-between">
                  ...
                  <span>{money(summary.taxPayableAmount)}</span>
                </div>
                <div className="flex justify-between">
                  <span>Tax Included</span>
                  <span>{money(summary.taxIncludedAmount)}</span>
                </div>
                <div className="flex justify-between">
                  <span>Delivery</span>
                  <span>
                    {money(
                      summary.deliveryChargeAmount ?? summary.shippingFeeAmount,
                    )}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span>Customer Platform Fee</span>
                  <span>{money(summary.customerPlatformFeeAmount)}</span>
                </div>
                <div className="flex justify-between">
                  <span>Platform Fee GST</span>
                  <span>{money(summary.customerPlatformFeeTaxAmount)}</span>
                </div>
                <div className="flex justify-between">
                  <span>COD Charge</span>
                  <span>{money(summary.codChargeAmount)}</span>
                </div>
                <div className="flex justify-between">
                  <span>Wallet</span>
                  <span>-{money(summary.walletDiscountAmount)}</span>
                </div>
                <div className="border-t pt-2 flex justify-between text-base font-semibold">
                  <span>Payable</span>
                  <span>{money(summary.customerPayableAmount)}</span>
                </div>
              </div>

              <div>
                <h3 className="mb-2 text-xs font-semibold uppercase text-gray-400">
                  Seller Settlement Preview
                </h3>
                  ...
                  {sellerSettlements.length ? (
                    sellerSettlements.map((seller) => (
                      ...
                      >
                        ...
                        <p>
                          Delivery: {money(seller.sellerDeliveryChargeAmount)}
                        </p>
                        <p>
                          Shipping reimbursement:{" "}
                          {money(seller.shippingReimbursementAmount)}
                        </p>
                        <p>
                          Shipping deduction:{" "}
                          {money(seller.shippingDeductionAmount)}
                        </p>
