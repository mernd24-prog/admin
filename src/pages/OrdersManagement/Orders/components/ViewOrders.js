...
const getItemKey = (item = {}) =>
  firstDefined(item.id, item._id, `${item.product_id}-${item.variant_sku}`);

const getReturnItemTitle = (item = {}) => {
  const snapshot = normalizeJson(
    firstDefined(item.productSnapshot, item.product_snapshot),
    {},
  );
  return firstDefined(
    item.productTitle,
...
const impactKeysForReturnItem = (item = {}) => {
  ...
  if (preciseKeys.length)
    return preciseKeys.map(([type, value]) => `${type}:${value}`);
...
const getReturnItemVariantText = (item = {}) => {
  ...
  const variant = normalizeJson(
    firstDefined(item.variantSnapshot, item.variant_snapshot, snapshot.variant),
    {},
  );
  return firstDefined(
    item.variantSku,
...
const buildReturnImpactByItem = (returnRequests = []) => {
  const impact = {};
  (Array.isArray(returnRequests) ? returnRequests : []).forEach(
    (returnRequest) => {
      if (!shouldCountReturnForPayout(returnRequest)) return;
      (Array.isArray(returnRequest.items) ? returnRequest.items : []).forEach(
        (item) => {
          ...
          keys.forEach((key) => {
            impact[key] = {
              ...
              refundAmount: money(
                (impact[key]?.refundAmount || 0) + refundAmount,
              ),
            };
          });
        },
...
const buildCancellationImpactByItem = (cancellations = []) => {
  const impact = {};
  (Array.isArray(cancellations) ? cancellations : []).forEach(
    (cancellation) => {
      const status = String(
        firstDefined(
          cancellation.status,
      ...
      const items = firstDefined(
        ...
        cancellation.cancellation_items,
        [],
      );
...
const buildSellerLookup = (relations = {}) => {
  ...
  (relations.sellerFulfillmentGroups || []).forEach((group) => {
    const organizationSnapshot = normalizeJson(
      group.organizationSnapshot || group.organization_snapshot,
      {},
    );
    remember(
  ...
  (relations.sellerSettlements || []).forEach((seller) => {
    const organizationSnapshot = normalizeJson(
      seller.organizationSnapshot || seller.organization_snapshot,
      {},
    );
    remember(
      seller.sellerId || seller.seller_id,
      seller.organizationId || seller.organization_id,
      {
  ...
  (relations.shipments || []).forEach((shipment) => {
    const sellerSnapshot = normalizeJson(
      shipment.sellerSnapshot || shipment.seller_snapshot,
      {},
    );
    const organizationSnapshot = normalizeJson(
      shipment.organizationSnapshot || shipment.organization_snapshot,
      {},
    );
    remember(
      shipment.sellerId ||
        ...
        shipment.seller?.id ||
        shipment.seller?._id,
      shipment.organizationId || shipment.organization_id,
      {
        sellerName:
          getSellerDisplayName(shipment.seller) ||
...
const getOrderTaxRates = (taxBreakup = {}, items = []) => {
  const sourceItems =
    Array.isArray(taxBreakup.items) && taxBreakup.items.length
      ? taxBreakup.items
      : items.map((item) =>
          normalizeJson(firstDefined(item.tax_breakup, item.taxBreakup), {}),
        );
...
const groupItemsBySeller = (items = [], relations = {}) => {
  ...
  return items.reduce((groups, item) => {
    ...
    const lookup =
      sellerLookup[`${sellerId}:${organizationId}`] ||
...
const sameSellerGroup = (entry = {}, group = {}) =>
  String(firstDefined(entry.sellerId, entry.seller_id, "")) ===
    String(group.sellerId || "") &&
  String(
    firstDefined(entry.organizationId, entry.organization_id, "default") ||
      "default",
  ) === String(group.organizationId || "default");
...
const buildSellerSettlements = (items = []) =>
  Object.values(groupItemsBySellerFromItems(items)).map((group) => {
    const totals = group.items.reduce(
      (acc, item) => {
        const itemTax = normalizeJson(
          firstDefined(item.tax_breakup, item.taxBreakup),
          {},
        );
        const pricing = normalizeJson(
          firstDefined(item.pricing_snapshot, item.pricingSnapshot),
          {},
        );
        ...
        const taxableAmount = money(
          firstDefined(
            ...
            itemTax.taxable_amount,
            lineTotal -
              money(firstDefined(item.discount_amount, item.discountAmount)),
          ),
        );
        ...
        const commissionFee = money(
          firstDefined(
            pricing.commissionFee,
            pricing.commission_fee,
            platformFee,
          ),
        );
        const commissionPercent = money(
          firstDefined(pricing.commissionPercent, pricing.commission_percent),
        );
    ...
    return {
      ...
      sellerPayout: Math.max(
        0,
        Number((totals.taxableSales - totals.platformFee).toFixed(2)),
      ),
    };
  });
...
const normalizeSellerSettlement = (seller = {}) => ({
  ...
  organizationName: firstDefined(
    seller.organizationName,
    seller.organization_name,
    seller.organizationSnapshot?.legalBusinessName,
    seller.organizationSnapshot?.storeDisplayName,
    "",
  ),
  grossSales: money(
    firstDefined(seller.grossSalesAmount, seller.gross_sales_amount, 0),
  ),
  taxableSales: money(
    firstDefined(
      seller.taxableAmount,
      seller.taxableSales,
      seller.taxable_sales,
      0,
    ),
  ),
  taxCollected: money(
    firstDefined(seller.taxAmount, seller.taxCollected, seller.tax_amount, 0),
  ),
  sellerPayoutBase: money(
    firstDefined(
      seller.sellerPayoutBaseAmount,
      seller.seller_payout_base_amount,
      seller.taxableAmount,
      seller.taxableSales,
      0,
    ),
  ...
  commissionFee: money(
    firstDefined(
      ...
      seller.commissionFee,
      seller.platform_fee_amount,
      0,
    ),
  ),
  commissionReversal: money(
    firstDefined(
      seller.commissionReversalAmount,
      seller.commission_reversal_amount,
      0,
    ),
  ),
  netCommissionFee: money(
    firstDefined(
      seller.netPlatformCommissionAmount,
      seller.net_platform_commission_amount,
      seller.platformFeeAmount,
      seller.commissionAmount,
      0,
    ),
  ),
  variableCommissionFee: money(
    firstDefined(seller.commissionFeeAmount, seller.commission_fee_amount, 0),
  ),
  platformFeeTax: money(
    firstDefined(
      seller.platformFeeTaxAmount,
      seller.commissionTaxAmount,
      seller.platform_fee_tax_amount,
      0,
    ),
  ),
  platformFeeTaxRate: money(
    firstDefined(
      seller.platformFeeTaxRate,
      seller.platform_fee_tax_rate,
      seller.commissionTaxRate,
      seller.commission_tax_rate,
      18,
    ),
  ),
  commissionTaxReversal: money(
    firstDefined(
      seller.commissionTaxReversalAmount,
      seller.commission_tax_reversal_amount,
      0,
    ),
  ),
  netCommissionTax: money(
    firstDefined(
      seller.netCommissionTaxAmount,
      seller.net_commission_tax_amount,
      seller.platformFeeTaxAmount,
      seller.commissionTaxAmount,
      0,
    ),
  ),
  commissionTaxableBase: money(
    firstDefined(
  ...
  discountAmount: money(
    firstDefined(seller.discountAmount, seller.discount_amount, 0),
  ),
  sellerFundedDiscount: money(
    firstDefined(
      seller.sellerFundedDiscountAmount,
      seller.seller_funded_discount_amount,
      0,
    ),
  ),
  marketplaceFundedDiscount: money(
    firstDefined(
      seller.marketplaceFundedDiscountAmount,
      seller.marketplace_funded_discount_amount,
      0,
    ),
  ),
  sellerDeliveryCharge: money(
    firstDefined(
      seller.sellerDeliveryChargeAmount,
      seller.seller_delivery_charge_amount,
      seller.deliveryChargeAmount,
      seller.delivery_charge_amount,
      0,
    ),
  ),
  shippingReimbursement: money(
    firstDefined(
      seller.shippingReimbursementAmount,
      seller.shipping_reimbursement_amount,
      0,
    ),
  ),
  shippingDeduction: money(
    firstDefined(
      seller.shippingDeductionAmount,
      seller.shipping_deduction_amount,
      0,
    ),
  ),
  shippingPolicy: firstDefined(
    seller.shippingPolicy,
    seller.shipping_policy,
    "",
  ),
  refundAmount: money(
    firstDefined(seller.refundAmount, seller.refund_amount, 0),
  ),
  sellerPayoutBaseReversal: money(
    firstDefined(
      seller.sellerPayoutBaseReversalAmount,
      seller.seller_payout_base_reversal_amount,
      seller.refundAmount,
  ...
  adjustmentAmount: money(
    firstDefined(seller.adjustmentAmount, seller.adjustment_amount, 0),
  ),
  gstTcsRate: money(firstDefined(seller.gstTcsRate, seller.gst_tcs_rate, 0)),
  gstTcsAmount: money(
    firstDefined(seller.gstTcsAmount, seller.gst_tcs_amount, 0),
  ),
  gstTcsTaxableBase: money(
    firstDefined(
      seller.gstTcsTaxableAmount,
      seller.gstTcsTaxableBaseAmount,
      seller.gst_tcs_taxable_base_amount,
      seller.taxableAmount,
      seller.taxableSales,
  ...
  gstTcsReversal: money(
    firstDefined(
      seller.gstTcsReversalAmount,
      seller.gst_tcs_reversal_amount,
      0,
    ),
  ),
  netGstTcsAmount: money(
    firstDefined(
      seller.netGstTcsAmount,
      seller.net_gst_tcs_amount,
      seller.gstTcsAmount,
      seller.gst_tcs_amount,
      0,
    ),
  ),
  gstTcsTaxableBaseReversal: money(
    firstDefined(
      seller.gstTcsTaxableBaseReversalAmount,
      seller.gst_tcs_taxable_base_reversal_amount,
      0,
    ),
  ),
  netGstTcsTaxableBase: money(
    firstDefined(
      seller.netGstTcsTaxableBaseAmount,
      seller.net_gst_tcs_taxable_base_amount,
      seller.taxableAmount,
  ...
  incomeTaxTdsRate: money(
    firstDefined(seller.incomeTaxTdsRate, seller.income_tax_tds_rate, 0),
  ),
  incomeTaxTdsTaxableBase: money(
    firstDefined(
      seller.incomeTaxTdsTaxableAmount,
      seller.incomeTaxTdsTaxableBaseAmount,
      seller.income_tax_tds_taxable_base_amount,
      seller.grossSalesAmount,
      seller.grossSales,
      0,
    ),
  ),
  incomeTaxTdsAmount: money(
    firstDefined(seller.incomeTaxTdsAmount, seller.income_tax_tds_amount, 0),
  ),
  incomeTaxTdsReversal: money(
    firstDefined(
      seller.incomeTaxTdsReversalAmount,
      seller.income_tax_tds_reversal_amount,
      0,
    ),
  ),
  incomeTaxTdsTaxableBaseReversal: money(
    firstDefined(
      seller.incomeTaxTdsTaxableBaseReversalAmount,
      seller.income_tax_tds_taxable_base_reversal_amount,
      0,
    ),
  ),
  netIncomeTaxTdsTaxableBase: money(
    firstDefined(
      seller.netIncomeTaxTdsTaxableBaseAmount,
      seller.net_income_tax_tds_taxable_base_amount,
      seller.grossSalesAmount,
      seller.grossSales,
      0,
    ),
  ),
  netIncomeTaxTdsAmount: money(
    firstDefined(
      seller.netIncomeTaxTdsAmount,
      seller.net_income_tax_tds_amount,
      seller.incomeTaxTdsAmount,
      seller.income_tax_tds_amount,
      0,
    ),
  ),
  sellerPayout: money(
  ...
  commissionStatus: firstDefined(
    seller.commissionStatus,
    seller.commission_status,
    "",
  ),
  payoutStatus: firstDefined(seller.payoutStatus, seller.payout_status, ""),
  payoutId: firstDefined(seller.payoutId, seller.payout_id, ""),
  payoutReference: firstDefined(
    seller.payoutReference,
    seller.payment_reference,
    "",
  ),
  payoutMethod: firstDefined(seller.payoutMethod, seller.payment_method, ""),
  payoutProcessedAt: firstDefined(
    seller.payoutProcessedAt,
    seller.processed_at,
    "",
  ),
  commissionRates: Array.isArray(seller.commissionRates)
    ? seller.commissionRates
    : [],
  commissionBreakdown: Array.isArray(seller.commissionBreakdown)
    ? seller.commissionBreakdown
    : [],
  commissionIds: Array.isArray(seller.commissionIds)
    ? seller.commissionIds
    : [],
});

const Panel = ({ title, children, actions, className = "" }) => (
  <section
    className={`rounded-lg border border-[#eadfbd] bg-[#fffdf8] shadow-[0_1px_3px_rgba(31,41,55,0.06)] ${className}`}
  >
    {(title || actions) && (
...
const InfoRow = ({ label, value, strong = false }) => (
  <div className="flex items-start justify-between gap-4 py-2 text-sm">
    ...
    <span
      className={`text-right text-[#202337] ${strong ? "font-semibold" : "font-medium"}`}
    >
...
const PayoutRow = ({
  ...
  return (
    <div
      className={`flex items-start justify-between gap-3 ${small ? "text-xs" : "text-sm"} ${toneClass} ${className}`}
    >
  ...
  return (
    <div className={`rounded-lg border px-3 py-3 ${toneClass}`}>
      <div className="mb-2">
        ...
        {subtitle && (
          <div className="mt-0.5 text-xs font-normal text-[#65718b]">
            {subtitle}
          </div>
        )}
      </div>
...
const commissionBaseNote = (seller = {}, commissionAmount = 0) => {
  ...
  const rate =
    seller.commissionRates.length === 1 ? money(seller.commissionRates[0]) : 0;
  const derivedBase =
...
const getOrderItemId = (item = {}) =>
  String(
    firstDefined(item.id, item._id, item.order_item_id, item.orderItemId, "") ||
      "",
  );
...
const getOrderItemTitle = (item = {}) => {
  const snapshot = normalizeJson(
    firstDefined(item.product_snapshot, item.productSnapshot),
    {},
  );
  return firstDefined(
    item.product_title,
...
const getOrderItemVariantLabel = (item = {}) => {
  ...
  if (directTitle && directTitle.toLowerCase() !== "default")
    return directTitle;

  const attributes = normalizeJson(
    firstDefined(
      item.attributes,
...
const findOrderItemForTaxLine = (taxItem = {}, items = [], index = 0) => {
  const taxPricingLineId = String(
    firstDefined(taxItem.pricingLineId, taxItem.pricing_line_id, "") || "",
  );
  const taxProductId = String(
    firstDefined(taxItem.productId, taxItem.product_id, "") || "",
  );
  const taxVariantId = String(
    firstDefined(taxItem.variantId, taxItem.variant_id, "") || "",
  );
  const taxVariantSku = String(
    firstDefined(taxItem.variantSku, taxItem.variant_sku, "") || "",
  );
  ...
  const exactMatch = items.find((item) => {
    ...
    const itemPricingLineId = String(
      firstDefined(
    ...
    const itemVariantId = String(
      firstDefined(item.variant_id, item.variantId, "") || "",
    );
    const itemVariantSku = String(
      firstDefined(item.variant_sku, item.variantSku, "") || "",
    );
...
const getReconciledItemTax = (item = {}) => {
  const saved = normalizeJson(
    firstDefined(item.tax_breakup, item.taxBreakup),
    {},
  );
  const lineTotal = money(
    firstDefined(item.line_total, item.lineTotal, saved.lineTotal, 0),
  );
  const savedLineTotal = money(
    firstDefined(saved.lineTotal, saved.line_total, lineTotal),
  );
  const gstRate = money(
    firstDefined(saved.gstRate, saved.gst_rate, item.gst_rate, item.gstRate, 0),
  );
  const cessRate = money(firstDefined(saved.cessRate, saved.cess_rate, 0));
  const gstInclusive =
    firstDefined(saved.gstInclusive, saved.gst_inclusive, true) !== false;
  const sellerFundedDiscount = money(
    firstDefined(
  ...
  const discountedLineTotal = money(
    Math.max(0, lineTotal - sellerFundedDiscount),
  );
...
const getCommissionForItem = (item = {}, records = []) => {
  ...
  const candidates = (Array.isArray(records) ? records : []).map((record) => {
    ...
    const variantIds = products
      .flatMap((product) => [product.variantId, product.variant_id])
      .filter(Boolean)
      .map(String);
    const variantSkus = products
      .flatMap((product) => [
        product.variantSku,
        product.variant_sku,
        product.sku,
    ...
    const productIds = products
      .flatMap((product) => [product.productId, product.product_id])
      .filter(Boolean)
      .map(String);
  ...
  return (
    candidates.find(
      (candidate) => itemId && candidate.itemIds.includes(String(itemId)),
    )?.record ||
    candidates.find(
      (candidate) =>
        (variantId && candidate.variantIds.includes(variantId)) ||
        (variantSku && candidate.variantSkus.includes(variantSku)),
    )?.record ||
    (candidates.filter(
      (candidate) =>
        productId && candidate.productIds.includes(String(productId)),
    ).length === 1
      ? candidates.find(
          (candidate) =>
            productId && candidate.productIds.includes(String(productId)),
...
const buildSimpleItemPayoutRows = (
  seller = {},
  sellerItems = [],
  commissionRecords = [],
  returnRequests = [],
  ...
  const productTotal = sellerItems.reduce(
    (total, item) =>
      total + money(firstDefined(item.line_total, item.lineTotal, 0)),
    0,
  );
  ...
  return sellerItems.map((item) => {
    ...
    const returnImpact =
      impactLookupKeys.map((key) => returnImpactByItem[key]).find(Boolean) ||
      {};
    const cancellationImpact =
      impactLookupKeys
        .map((key) => cancellationImpactByItem[key])
        .find(Boolean) || {};
    const returnedQuantity = Math.min(
      itemQuantity,
      money(returnImpact.quantity || 0),
    );
    const directCancelledQuantity = money(
      firstDefined(item.cancelled_quantity, item.cancelledQuantity, 0),
    );
    const cancelledQuantity = Math.min(
      Math.max(0, itemQuantity - returnedQuantity),
      Math.max(
        directCancelledQuantity,
        money(cancellationImpact.quantity || 0),
      ),
    );
    const reversedQuantity = Math.min(
      itemQuantity,
      money(returnedQuantity + cancelledQuantity),
    );
    const remainingQuantity = Math.max(
      0,
      money(itemQuantity - reversedQuantity),
    );
    const returnRatio =
      itemQuantity > 0 ? Math.min(1, reversedQuantity / itemQuantity) : 0;
    const tax = getReconciledItemTax(item);
    const pricing = normalizeJson(
      firstDefined(item.pricing_snapshot, item.pricingSnapshot),
      {},
    ...
    const metadata = normalizeJson(record?.metadata, {});
    const product = Array.isArray(metadata.products)
      ? metadata.products[0] || {}
      : {};
    const lineTotal = money(
      firstDefined(
    ...
    const financialRecordFinalized = [
      "paid",
      "completed",
      "processed",
      "settled",
    ].includes(recordStatus);
    const useReconciledLine =
      tax.reconciliationRequired && !financialRecordFinalized;
    const gstTcsTaxableBase = useReconciledLine
      ? taxableBase
      : money(
          firstDefined(
            metadata.taxableSupplyAmount,
            metadata.gstTcsTaxableAmount,
            metadata.gst_tcs_taxable_amount,
            pricing.gstTcsTaxableAmount,
            pricing.gst_tcs_taxable_amount,
            taxableBase,
          ),
        );
    const incomeTaxTdsTaxableBase = useReconciledLine
      ...
      : money(
          firstDefined(
            metadata.incomeTaxTdsTaxableAmount,
            metadata.income_tax_tds_taxable_amount,
            pricing.incomeTaxTdsTaxableAmount,
            pricing.income_tax_tds_taxable_amount,
            taxableBase,
          ),
        );
    const productShare = productTotal > 0 ? lineTotal / productTotal : 0;
    const taxableShare =
      taxableTotal > 0 ? taxableBase / taxableTotal : productShare;
    const payoutMode = String(
      firstDefined(
        pricing.sellerPayoutBase,
        pricing.seller_payout_base,
        metadata.sellerPayoutBase,
        "",
      ),
    ).toLowerCase();
    const discountIncluded =
      payoutMode === "gross_customer_price" ||
      payoutMode === "gross_seller_invoice";
    ...
    const productAmount = money(
      firstDefined(
        record?.amount,
        product.grossSellerInvoiceAmount,
        product.amount,
        pricing.sellerPayoutBaseAmount,
        pricing.seller_payout_base_amount,
        lineTotal,
      ),
    );
    const shipping = money(
      firstDefined(
        pricing.shippingReimbursementAmount,
        pricing.shipping_reimbursement_amount,
        metadata.shippingReimbursementAmount,
        metadata.sellerDeliveryChargeAmount,
        seller.shippingReimbursement * productShare,
      ),
    );
    const shippingDeduction = money(
      firstDefined(
        pricing.shippingDeductionAmount,
        pricing.shipping_deduction_amount,
        metadata.shippingDeductionAmount,
        seller.shippingDeduction * productShare,
      ),
    );
    const discount = discountIncluded
      ? 0
      : money(
          firstDefined(
            pricing.marketplaceFundedDiscountAmount,
            pricing.marketplace_funded_discount_amount,
            product.marketplaceFundedDiscountAmount,
            seller.marketplaceFundedDiscount * productShare,
          ),
        );
    const commissionRate = money(
      firstDefined(
        metadata.platformFeeRate,
        metadata.commissionRate,
        pricing.platformFeeRate,
        pricing.commissionRate,
        seller.commissionRates?.[0],
        0,
      ),
    );
    const commission =
      useReconciledLine && commissionRate > 0
        ...
        : money(
            firstDefined(
              ...
              metadata.commissionFeeAmount,
              product.platformFeeAmount,
              pricing.platformFeeAmount,
              seller.commissionFee * taxableShare,
            ),
          );
    const commissionGstRate = money(
      firstDefined(
        metadata.platformFeeTaxRate,
        metadata.commissionTaxRate,
        pricing.platformFeeTaxRate,
        pricing.commissionTaxRate,
        seller.platformFeeTaxRate,
        18,
      ),
    );
    const commissionGst = useReconciledLine
      ...
      : money(
          firstDefined(
            record?.tax_amount,
            record?.taxAmount,
            metadata.platformFeeTaxAmount,
            product.platformFeeTaxAmount,
            pricing.platformFeeTaxAmount,
            seller.platformFeeTax * taxableShare,
          ),
        );
    const commissionBase = money(
      firstDefined(
    ...
    const gstTcs = useReconciledLine
      ...
      : money(
          firstDefined(
            metadata.gstTcsAmount,
            pricing.gstTcsAmount,
            pricing.gst_tcs_amount,
            seller.gstTcsAmount * taxableShare,
          ),
        );
    const incomeTaxTds = useReconciledLine
      ...
      : money(
          firstDefined(
            metadata.incomeTaxTdsAmount,
            pricing.incomeTaxTdsAmount,
            pricing.income_tax_tds_amount,
            seller.incomeTaxTdsAmount * taxableShare,
          ),
        );
    ...
    const netIncomeTaxTdsTaxableBase = money(
      incomeTaxTdsTaxableBase * (1 - returnRatio),
    );
    const computedOriginalPayout = money(
      productAmount +
        shipping +
        discount -
        shippingDeduction -
    ...
    const backendRefundRecovery = money(
      firstDefined(
    ...
    const refundRecovery =
      backendRefundRecovery ||
      (returnRatio > 0 ? money(computedOriginalPayout * returnRatio) : 0);
    const backendNetPayout = money(
      firstDefined(record?.net_amount, record?.netAmount, 0),
    );
    const finalPayout =
      record && !useReconciledLine
        ? refundRecovery > 0 && backendRefundRecovery <= 0
          ? Math.max(0, money(backendNetPayout - refundRecovery))
          : backendNetPayout
        : Math.max(
            0,
            money(
    ...
    const nonShippingOriginalPayout = money(
      computedOriginalPayout - originalShippingNet,
    );
    const estimatedFinalWithoutShipping = money(
      nonShippingOriginalPayout * (1 - returnRatio),
    );
    const finalShippingNet = money(
      Math.max(
        Math.min(
          finalPayout - estimatedFinalWithoutShipping,
          Math.max(originalShippingNet, 0),
        ),
        Math.min(originalShippingNet, 0),
      ),
    );
    const shippingNetReversal = money(originalShippingNet - finalShippingNet);
    const netShipping = originalShippingNet >= 0 ? finalShippingNet : 0;
    const netShippingDeduction =
      originalShippingNet < 0
        ? Math.abs(finalShippingNet)
        : money(shippingDeduction * (1 - returnRatio));
    const shippingReversal =
      shipping > 0 ? Math.max(0, money(shipping - netShipping)) : 0;
    const shippingDeductionReversal =
      shippingDeduction > 0
    ...
    return {
      ...
      status:
        remainingQuantity <= 0 && isCancelled
          ? "Cancelled"
      ...
      customerRefundAmount: money(
        (returnImpact.refundAmount || 0) +
          (cancellationImpact.refundAmount || 0),
      ),
      productAmount,
      shipping,
      shippingDeduction,
...
const SimpleItemPayoutBreakup = ({ rows = [], finalTotal = 0 }) => (
  <div className="overflow-hidden rounded-xl border border-[#eadfbd] bg-white">
    <div className="bg-[#fff9ea] px-4 py-3">
      <div className="text-sm font-bold text-[#202337]">
        Seller payout by product
      </div>
      <div className="mt-1 text-xs text-[#65718b]">
        Simple view: original payout, reversed amount, and final payable.
      </div>
    </div>
                  ...
                  {row.orderedQuantity > 1 && (
                    <span>
                      ...
                      {row.returnedQuantity > 0
                        ? ` · Returned ${row.returnedQuantity} · Payable ${row.remainingQuantity}`
                        : ""}
                      {row.cancelledQuantity > 0
                        ? ` · Cancelled ${row.cancelledQuantity} · Payable ${row.remainingQuantity}`
                        : ""}
                    </span>
                  )}
                </div>
            ...
            <div className="space-y-2 rounded-lg bg-[#fffdf8] px-3 py-2">
              <div className="grid gap-2 sm:grid-cols-3">
                <div className="rounded-lg bg-white p-2">
                  <div className="text-[11px] font-bold uppercase text-[#65718b]">
                    Original payout
                  </div>
                ...
                <div className="rounded-lg bg-[#fff7ed] p-2">
                  <div className="text-[11px] font-bold uppercase text-[#9a5b00]">
                    Reversed
                  </div>
                ...
                <div className="rounded-lg bg-[#eefbf2] p-2">
                  <div className="text-[11px] font-bold uppercase text-[#21812C]">
                    Final payable
                  </div>
              ...
              {(row.shipping > 0 || row.shippingDeduction > 0) && (
                <div className="rounded-lg border border-[#d9f1df] bg-[#f7fff9] px-3 py-2 text-xs">
                  <div className="mb-1 font-bold text-[#21812C]">
                    Shipping settlement
                  </div>
                  {row.shipping > 0 && (
                    <PayoutRow
...
const ItemWisePayoutCalculation = ({ rows = [], totals = {} }) => (
  <div className="space-y-3 border-t border-[#dbe6f7] p-3">
    {rows.map((row, index) => (
      <div
        key={row.id || index}
        className="overflow-hidden rounded-lg border border-[#dbe6f7] bg-white"
      >
        <div className="flex flex-wrap items-start justify-between gap-3 bg-[#f8faff] px-3 py-2">
          ...
          <div className="text-right">
            <div className="text-[11px] font-semibold uppercase text-[#65718b]">
              Final item payout
            </div>
            <div className="text-base font-bold text-[#21812C]">
              {formatMoney(row.finalPayout)}
            ...
            <tbody className="divide-y divide-[#edf1f7] text-[#202337]">
              <tr>
                <td className="px-3 py-2">
                  ...
                  <div className="text-[#65718b]">
                    Payable amount for this product.
                  </div>
                </td>
              ...
              {row.shippingDeduction > 0 && (
                <tr>
                  <td className="px-3 py-2">
                    <div className="font-semibold">
                      Shipping deducted from seller
                    </div>
                    <div className="text-[#65718b]">
                      Configured seller-side shipping deduction.
                    </div>
                  </td>
                  <td className="px-3 py-2 text-right font-semibold">
    ...
    <div className="overflow-hidden rounded-lg border border-[#cdebd6] bg-[#f7fff9]">
      ...
      <div className="grid gap-2 p-3 text-sm sm:grid-cols-3">
        <div>
          <div className="text-xs font-bold uppercase text-[#65718b]">
            Original payout
          </div>
        ...
        <div>
          <div className="text-xs font-bold uppercase text-[#b45309]">
            Return removed
          </div>
        ...
        <div>
          <div className="text-xs font-bold uppercase text-[#21812C]">
            Final payout
          </div>
      ...
      <div className="border-t border-[#d9f1df] px-3 py-2 text-xs leading-5 text-[#65718b]">
        Final payout is the amount payable to the seller for
        delivered/non-returned quantity after platform commission, GST on
        commission, GST TCS, income-tax TDS, shipping settlement, and return
        reversal.
      </div>
    </div>
  </div>
  ...
  <div className="rounded-lg border border-[#eadfbd] bg-white p-4 text-sm">
    <div className="mb-3 flex items-start justify-between gap-3">
      <div>
        ...
        {subtitle && (
          <div className="mt-0.5 text-xs text-[#65718b]">{subtitle}</div>
        )}
      </div>
...
const ShipmentCard = ({ shipment = {}, seller, onManage, onDownloadLabel }) => {
  ...
  const eventCount = Array.isArray(shipment.trackingEvents)
    ? shipment.trackingEvents.length
    : 0;
  const createdAt = formatDate(
    firstDefined(shipment.created_at, shipment.createdAt),
  );
    ...
    <div className="overflow-hidden rounded-xl border border-[#eadfbd] bg-white shadow-[0_1px_4px_rgba(31,41,55,0.04)]">
      <div className="flex flex-wrap items-start justify-between gap-3 bg-[#fff9ea] px-4 py-3">
        <div className="min-w-0">
          ...
          {seller && (
            <div className="mt-1 text-xs text-[#65718b]">{seller}</div>
          )}
        </div>
      ...
      <div className="grid grid-cols-1 gap-2 p-3 sm:grid-cols-2">
        ...
        <DetailStat
          label="Tracking / AWB"
          value={trackingNumber || "Will be added after dispatch"}
          tone={trackingNumber ? "default" : "warning"}
        />
        <DetailStat
          label="Collection"
          value={isCod ? "COD collection" : "Prepaid / no COD"}
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 border-t border-[#f1e7cd] px-4 py-3 text-xs text-[#65718b]">
        <span>
          {eventCount
            ? `${eventCount} tracking update${eventCount === 1 ? "" : "s"}`
    ...
    <div className="overflow-hidden rounded-xl border border-[#eadfbd] bg-white shadow-[0_1px_4px_rgba(31,41,55,0.04)]">
      ...
      <div className="grid grid-cols-1 gap-2 p-3 sm:grid-cols-2">
        ...
        <DetailStat
          label="Created"
          value={formatDate(
            firstDefined(payment.created_at, payment.createdAt),
          )}
          tone="muted"
        />
      </div>
...
const OrderSummary = () => {
  ...
  const fetchOrderReturns = useCallback(async () => {
    ...
    try {
      const res = await dispatch(
        getAdminReturns({ orderId: id, limit: 20 }),
      ).unwrap();
    ...
    async (shipment) => {
      ...
      try {
        await downloadApiFile(
          ENDPOINTS.orders.boxLabelDownload(orderId, shipmentId),
          { format: "pdf" },
        );
  ...
  const relationActions = [
    ...
    { label: "Returns", module: "returns", path: "/app/returns" },
    {
      ...
      path: "/app/cancellations",
    },
    {
      label: "Tax Invoices",
      module: "tax-invoices",
      path: "/app/tax-invoices",
    },
    {
      label: "Credit Notes",
      module: "credit-notes",
  ...
  const taxBreakup = normalizeJson(
    firstDefined(order.tax_breakup, order.taxBreakup),
    {},
  );
  const summary = order.summary || {};
  const taxIncludedAmount = money(
    firstDefined(summary.taxIncludedAmount, taxBreakup.taxIncludedAmount),
  );
  const taxPayableAmount = money(
    firstDefined(summary.taxPayableAmount, taxBreakup.taxPayableAmount),
  );
  const customerPlatformFeeAmount = money(
    firstDefined(summary.customerPlatformFeeAmount, 0),
  );
  const customerPlatformFeeTaxAmount = money(
    firstDefined(summary.customerPlatformFeeTaxAmount, 0),
  );
  ...
  const subtotalAmount = money(
    firstDefined(
      ...
      items.reduce(
        (total, item) =>
          total + money(firstDefined(item.line_total, item.lineTotal)),
        0,
      ),
    ),
  );
  ...
  const discountLabel =
    discountSource === "influencer"
      ? "Influencer Discount"
      : "Platform Discount";
  const deliveryChargeAmount = money(
    firstDefined(
  ...
  const codChargeAmount = money(
    firstDefined(
      summary.codChargeAmount,
      order.cod_charge_amount,
      order.codChargeAmount,
    ),
  );
  const walletDiscountAmount = money(
    firstDefined(
      summary.walletDiscountAmount,
  ...
  const customerTotalAmount = Number(
    Math.max(
      0,
      subtotalAmount -
        discountAmount +
  ...
  const customerPayableAmount = Number(
    Math.max(0, customerTotalAmount - walletDiscountAmount).toFixed(2),
  );
  ...
  const shipments = Array.isArray(relations.shipments)
    ? relations.shipments
    : [];
  const walletTransactions = Array.isArray(relations.walletTransactions)
    ? relations.walletTransactions
    : [];
  const cancellations = Array.isArray(relations.cancellations)
    ? relations.cancellations
    : [];
  const returns = Array.isArray(state.returns) ? state.returns : [];
  const hasPendingItemRequest =
    cancellations.some(
      (request) =>
    ...
    returns.some((request) =>
      ["requested", "pending", "under_review"].includes(
        String(request.status || "").toLowerCase(),
      ),
    );
  ...
  const sellerGroups = useMemo(
    () => Object.values(groupItemsBySeller(items, relations)),
    [items, relations],
  );
  const sellerSettlements = useMemo(() => {
    const savedSettlements = Array.isArray(relations.sellerSettlements)
  ...
  const statusOptions = useMemo(() => {
    if (isSeller) return [];
    const currentStatus = normalizeStatusKey(
      firstDefined(order.status, order.order_status),
    );
    ...
    const isAdminOrderManager =
      isSuperAdmin || isAdmin || ADMIN_ORDER_ROLES.has(normalizedRole);
    const paymentStatus = normalizePaymentStatusKey(
      firstDefined(order.payment_status, order.paymentStatus),
    );
    const paymentProvider = normalizePaymentStatusKey(
      firstDefined(order.payment_provider, order.paymentProvider),
    );
    const paidForFulfillment =
      paymentStatus === "captured" ||
      (paymentProvider === "cod" && paymentStatus === "authorized");
    const filteredNext = validNext.filter((s) => {
      if (!paidForFulfillment && PAYMENT_REQUIRED_STATUSES.has(s)) return false;
      if (isAdminOrderManager) return true;
      return true;
    });
    const fallbackNext = ORDER_STATUS_VALUES.filter(
      (status) =>
        status !== currentStatus &&
        (paidForFulfillment || !PAYMENT_REQUIRED_STATUSES.has(status)),
    );
    const transitionOptions = (
      validNext.length ? filteredNext : fallbackNext
    ).map(toOption);

    return [
      ...new Map(
        transitionOptions.map((option) => [option.value, option]),
      ).values(),
    ];
  }, [
    order,
  ...
  const openSellerCancellationModal = useCallback(() => {
    setFormData((prev) => ({
      ...
      refundMethod: "auto",
      cancelItems: Object.fromEntries(
        items
          .map((item) => [
            String(item.id),
  ...
  const handleStatusSubmit = useCallback(async () => {
    ...
    if (
      ...
      formData.reason.trim().length < MINIMUM_CANCEL_REASON_LENGTH
    ) {
      toast.error(
        `Please provide a cancellation reason with at least ${MINIMUM_CANCEL_REASON_LENGTH} characters`,
      );
      return;
    }
      ...
      <div>
            ...
            <>
              <PermissionGuard module="orders" action="update" hide>
                <button
                  type="button"
                  onClick={() =>
                    setState((prev) => ({ ...prev, noteModal: true }))
                  }
                >
        ...
        <div className="mb-4 grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
          ...
          <MetricCard
            ...
            value={
              <StatusBadge
                status={firstDefined(order.payment_status, order.paymentStatus)}
                dot
              />
            }
          />
          <MetricCard
            label="Order Status"
            value={<StatusBadge status={displayedOrderStatus} dot />}
          />
        ...
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
              ...
              sellerGroups.map((group) =>
                (() => {
                  const fulfillmentGroup =
                    (relations.sellerFulfillmentGroups || []).find((entry) =>
                      sameSellerGroup(entry, group),
                    ) || {};
                  const groupShipments = shipments.filter(
                    (shipment) =>
                      sameSellerGroup(shipment, group) &&
                      String(shipment.direction || "forward") !== "reverse",
                  );
                  const groupSettlement =
                    sellerSettlements.find((settlement) =>
                      sameSellerGroup(settlement, group),
                    ) || {};
                  ...
                  const returnLifecycle =
                    fulfillmentGroup.returnLifecycle || {};
                  return (
                    <div
                      key={`${group.sellerId}-${group.organizationId}`}
                      className="mb-4 overflow-hidden rounded-lg border border-[#eadfbd] bg-white last:mb-0"
                    >
                      <div className="flex flex-wrap items-center justify-between gap-2 bg-[#fff9ea] px-4 py-3">
                        <div>
                          {canOpenAdminProfiles && group.sellerId ? (
                            <DetailLink
                              onClick={() =>
                                navigate(`/app/seller/view/${group.sellerId}`)
                              }
                            ...
                            <span className="text-sm font-semibold text-[#202337]">
                              {group.organizationName || group.sellerName}
                            </span>
                          )}
                          ...
                          <div className="mt-2 flex flex-wrap gap-2 text-xs">
                            <span className="rounded-full bg-white px-2.5 py-1 text-[#202337] ring-1 ring-[#eadfbd]">
                              Package:{" "}
                              {formatLabel(displayStatus(packageStatus))}
                            </span>
                            <span className="rounded-full bg-white px-2.5 py-1 text-[#202337] ring-1 ring-[#eadfbd]">
                              Returns:{" "}
                              {formatLabel(
                                displayStatus(
                                  firstDefined(returnLifecycle.status, "none"),
                                ),
                              )}
                            </span>
                            <span className="rounded-full bg-white px-2.5 py-1 text-[#202337] ring-1 ring-[#eadfbd]">
                              ...
                              {formatLabel(
                                displayStatus(
                                  firstDefined(
                                    groupSettlement.payoutStatus,
                                    groupSettlement.commissionStatus,
                                    "pending",
                                  ),
                                ),
                              )}
                            ...
                            {groupSettlement.eligibleAt && (
                              <span className="rounded-full bg-white px-2.5 py-1 text-[#202337] ring-1 ring-[#eadfbd]">
                                Eligible:{" "}
                                {formatDate(groupSettlement.eligibleAt)}
                              </span>
                            )}
                          </div>
                        </div>
                        <div className="flex flex-wrap items-center justify-end gap-2">
                          {paidForFulfillment ? (
                            <button
                              ...
                              onClick={() =>
                                navigate(
                                  `/app/shipment-tracking?orderId=${encodeURIComponent(orderId)}&sellerId=${encodeURIComponent(group.sellerId || "")}`,
                                )
                              }
                            >
                              Manage Shipment
                      ...
                      {group.items.map((item) => {
                        ...
                        const cancellationDisplayStatus =
                          itemCancellation?.status === "requested"
                            ? "cancellation_requested"
                            : itemCancellation?.status === "rejected"
                              ? null
                              : itemCancellation?.status &&
                                  itemCancellation.status !== "completed"
                                ? "cancellation_approved"
                                : null;
                        const productSnapshot = normalizeJson(
                          firstDefined(
                        ...
                        return (
                          <div
                            key={getItemKey(item)}
                            className="grid grid-cols-1 gap-3 border-b border-[#eef0f6] px-4 py-4 text-sm last:border-b-0 md:grid-cols-12 md:items-start"
                          >
                            <div className="md:col-span-5">
                              {productId ? (
                                <DetailLink
                                  onClick={() =>
                                    navigate(
                                      `/app/product-catalog/view/${productId}`,
                                    )
                                  }
          ...
          <aside className="space-y-4">
            <Panel
              title={
                isSeller ? "Seller Order Summary" : "Customer Payment Breakup"
              }
            >
              <InfoRow
                label="Order Date"
              ...
              {!isSeller && walletDiscountAmount > 0 && (
                <InfoRow
        ...
        <div className="mt-4 space-y-4">
          <Panel title="GST Summary">
            ...
            {Array.isArray(taxBreakup.items) && taxBreakup.items.length > 0 && (
              <div className="mt-3">
                <div className="mb-2 text-xs font-semibold uppercase text-[#65718b]">
                  Item-wise GST
                </div>
                <div className="grid grid-cols-1 gap-2 md:grid-cols-2">
                  {taxBreakup.items.map((taxItem, index) => {
                    const taxProductId = firstDefined(
                      taxItem.productId,
                      taxItem.product_id,
                    );
                    const orderItem = findOrderItemForTaxLine(
                      taxItem,
                      items,
                      index,
                    );
                    ...
                    const baseProductTitle = firstDefined(
                    ...
                    const itemGstAmount =
                      money(taxItem.taxAmount) + money(taxItem.cessAmount);
                    return (
                      <div
                        key={`${productId || "tax"}-${index}`}
                        className="rounded-md border border-[#e6ebf7] bg-white p-3 text-xs"
                      >
          ...
          <Panel title="Customer & Delivery Address">
            <div className="grid grid-cols-1 gap-4 md:grid-cols-[minmax(220px,320px)_1fr]">
              ...
              <div className="rounded-md bg-[#f8faff] p-3 text-sm leading-6 text-[#202337]">
                <div className="mb-1 text-xs font-semibold uppercase text-[#65718b]">
                  Delivery address
                </div>
                <div>
                  {[
            ...
            <div className="flex flex-wrap gap-2">
              {relationActions.map((action) => (
                <button
                  ...
                  onClick={() =>
                    navigate(
                      `${action.path}?orderId=${encodeURIComponent(orderId)}`,
                    )
                  }
                >
                  {action.label}
                  ...
                  {cancellations.map((cancellation) => (
                    <RelatedCard
                      ...
                      rows={[
                        ...
                        {
                          label: "Refund status",
                          value: displayStatus(cancellation.refund_status),
                        },
                        {
                          label: "Inventory",
                          value: displayStatus(cancellation.inventory_status),
                        },
                        {
                          label: "Shipment",
                          value: displayStatus(cancellation.shipment_status),
                        },
                        {
                          label: "Requested",
                          value: formatDate(cancellation.created_at),
                        },
                      ]}
                      action={
                        <button
                          ...
                          return (
                            <RelatedCard
                              ...
                              subtitle={displayStatus(
                                formatLabel(returnRequest.reason),
                              )}
                              ...
                              rows={[
                                ...
                                {
                                  label: "Seller payout impact",
                                  value: "Only returned quantity is reversed",
                                },
                                {
                                  label: "Credit note / reverse invoice",
                                  value: creditNoteLabel,
                                },
                                {
                                  label: "Requested",
                                  value: formatDate(
                                  ...
                                  <div className="space-y-2 rounded-md  bg-[#fffaf0] p-2 text-xs">
                                    ...
                                    {returnRequest.items.map((item, index) => (
                                      <div
                                        ...
                                        className="flex items-start justify-between gap-3 border-t border-[#efe6cd] pt-2 first:border-t-0 first:pt-0"
                                      >
                                        <div className="min-w-0">
          ...
          {sellerSettlements.length ? (
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
              {sellerSettlements.map((seller) => {
                const sellerItems =
                  sellerGroups.find((group) => sameSellerGroup(seller, group))
                    ?.items || [];
                const sellerCommissionRecords = (
                  relations.sellerCommissions || []
                ).filter(
                  (record) =>
                    String(
                      firstDefined(record.seller_id, record.sellerId, ""),
                    ...
                    (!seller.organizationId ||
                      ...
                      ) === String(seller.organizationId || "default")),
                );
                const productRows = buildSimpleItemPayoutRows(
                  seller,
                  sellerItems,
                ...
                const itemRefundRecovery = sumMoney(
                  productRows,
                  "refundRecovery",
                );
                const hasItemLevelAdjustment = productRows.some(
                  (row) =>
                    money(row.refundRecovery) > 0 ||
                    money(row.returnedQuantity) > 0 ||
                    money(row.cancelledQuantity) > 0,
                );
                const displaySellerPayout = hasProductRows
                  ? productRowsTotal
                  : seller.sellerPayout;
                const payoutMismatch =
                  hasProductRows &&
                  Math.abs(productRowsTotal - seller.sellerPayout) >= 0.01;
                const itemProductPayable = sumMoney(
                  productRows,
                  "productAmount",
                );
                const itemShipping = sumMoney(productRows, "shipping");
                const itemShippingDeduction = sumMoney(
                  productRows,
                  "shippingDeduction",
                );
                const itemDiscount = sumMoney(productRows, "discount");
                const itemCommission = sumMoney(productRows, "netCommission");
                const itemCommissionTax = sumMoney(
                  productRows,
                  "netCommissionGst",
                );
                const itemGstTcs = sumMoney(productRows, "netGstTcs");
                const itemIncomeTaxTds = sumMoney(
                  productRows,
                  "netIncomeTaxTds",
                );
                const itemGstTcsBase = sumMoney(
                  productRows,
                  "netGstTcsTaxableBase",
                );
                const itemIncomeTaxTdsBase = sumMoney(
                  productRows,
                  "netIncomeTaxTdsTaxableBase",
                );
                const itemTaxCollected = sumMoney(productRows, "taxAmount");
                const beforeReturnTotal = sumMoney(productRows, "beforeReturn");
                const productPayable = hasProductRows
                  ? itemProductPayable
                  : seller.sellerPayoutBase || seller.grossSales;
                const netCommission =
                  seller.commissionReversal > 0
                    ? seller.netCommissionFee
                    : seller.commissionFee;
                const netCommissionTax =
                  seller.commissionTaxReversal > 0
                    ? seller.netCommissionTax
                    : seller.platformFeeTax;
                const netGstTcs =
                  seller.gstTcsReversal > 0
                    ? seller.netGstTcsAmount
                    : seller.gstTcsAmount;
                const netIncomeTaxTds =
                  seller.incomeTaxTdsReversal > 0
                    ? seller.netIncomeTaxTdsAmount
                    : seller.incomeTaxTdsAmount;
                const displayShipping = hasProductRows
                  ? itemShipping
                  : seller.shippingReimbursement;
                const displayShippingDeduction = hasProductRows
                  ? itemShippingDeduction
                  : seller.shippingDeduction;
                const displayMarketplaceDiscount = hasProductRows
                  ? itemDiscount
                  : seller.marketplaceFundedDiscount;
                const displayCommission = hasProductRows
                  ? itemCommission
                  : netCommission;
                const displayCommissionTax = hasProductRows
                  ? itemCommissionTax
                  : netCommissionTax;
                const displayGstTcs = hasProductRows ? itemGstTcs : netGstTcs;
                const displayIncomeTaxTds = hasProductRows
                  ? itemIncomeTaxTds
                  : netIncomeTaxTds;
                ...
                const displayIncomeTaxTdsBase = hasProductRows
                  ...
                  : seller.incomeTaxTdsReversal > 0
                    ? firstDefined(
                        seller.netIncomeTaxTdsTaxableBase,
                        seller.incomeTaxTdsTaxableBase,
                      )
                    : seller.incomeTaxTdsTaxableBase;
                const hasRefundAdjustment =
                ...
                return (
                  <div
                    key={`${seller.sellerId}-${seller.organizationId || "default"}`}
                    className="rounded-xl border border-[#eadfbd] bg-white p-4 text-sm shadow-[0_1px_4px_rgba(31,41,55,0.05)]"
                  >
                    <div className="mb-4 flex flex-wrap items-start justify-between gap-3">
                      <div>
                        {seller.sellerId && canOpenAdminProfiles ? (
                          <DetailLink
                            onClick={() =>
                              navigate(`/app/seller/view/${seller.sellerId}`)
                            }
                            className="font-semibold"
                          >
                            {seller.sellerName}
                          ...
                          <span className="font-semibold text-[#202337]">
                            {seller.sellerName}
                          </span>
                        )}
                        {seller.organizationName && (
                        ...
                        {hasRefundAdjustment && (
                          <div className="mt-2 inline-flex rounded-full bg-[#fff3d6] px-2.5 py-1 text-xs font-semibold text-[#8A5A00]">
                            Payout adjusted because an item was returned or
                            cancelled
                          </div>
                        )}
                      </div>
                    ...
                    <div className="space-y-3">
                      {productRows.length > 0 ? (
                        <SimpleItemPayoutBreakup
                          rows={productRows}
                          finalTotal={displaySellerPayout}
                        />
                      ) : (
                        <PayoutFormula>
                          No product-wise payout records found. Showing
                          seller-level payout below.
                        </PayoutFormula>
                      )}
                        ...
                        <div className="flex flex-wrap gap-x-3 gap-y-1 rounded-md bg-[#f8faff] px-3 py-2 text-xs text-[#65718b]">
                          {seller.commissionStatus && (
                            <span>
                              Commission:{" "}
                              {formatLabel(
                                displayStatus(seller.commissionStatus),
                              )}
                            </span>
                          )}
                          {seller.payoutStatus && (
                            <span>
                              Payout:{" "}
                              {formatLabel(displayStatus(seller.payoutStatus))}
                            </span>
                          )}
                          {seller.payoutMethod && (
                            <span>
                              Method:{" "}
                              {formatLabel(displayStatus(seller.payoutMethod))}
                            </span>
                          )}
                          {seller.payoutReference && (
                            <span>Bank ref: {seller.payoutReference}</span>
                          )}
                          {seller.payoutProcessedAt && (
                            <span>
                              Paid: {formatDate(seller.payoutProcessedAt)}
                            </span>
                          )}
                          {seller.commissionIds.length > 0 && (
                            <span>
                              Item records: {seller.commissionIds.length}
                            </span>
                          )}
                        </div>
                      )}
              ...
              <div className="space-y-2">
                {timeline.map((entry) => {
                  const statusLabel = formatLabel(
                    displayStatus(entry.to_status || entry.toStatus),
                  );
                  ...
                  return (
                    <div
                      key={entry.id}
                      className="rounded-md border border-[#e6ebf7] bg-white px-3 py-2 text-sm"
                    >
                      <div className="flex flex-wrap items-start justify-between gap-2">
                        <div>
                          <div className="font-semibold capitalize text-[#202337]">
                            {statusLabel}
                          </div>
                          {reasonLabel && (
                            <div className="mt-0.5 text-xs text-[#65718b]">
          ...
          <Panel title="Notes" className="mt-4">
            {notes.length ? (
              notes.map((note) => (
                <div
                  key={note.id}
                  className="mb-3 rounded-lg border border-[#eadfbd] bg-white p-3 text-sm last:mb-0"
                >
                  <div className="text-[#202337] leading-6">{note.note}</div>
                  <div className="text-xs text-[#65718b] mt-1">
                    {formatLabel(note.actor_role || "system")} ·{" "}
                    {formatLabel(note.visibility)} ·{" "}
                ...
                value={{
                  ...
                  label:
                    noteData.visibility === "internal"
                      ? "Internal"
                      : noteData.visibility === "seller"
                        ? "Seller visible"
                        : "Buyer visible",
                }}
                options={[
                  { value: "internal", label: "Internal" },
                  { value: "seller", label: "Seller visible" },
                  { value: "buyer", label: "Buyer visible" },
                ]}
                onChange={(option) =>
                  setNoteData((prev) => ({
                    ...prev,
                    visibility: option?.value || "internal",
                  }))
                }
