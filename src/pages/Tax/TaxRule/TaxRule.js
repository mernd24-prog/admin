...
const TaxRule = () => {
  ...
  const getListTaxRuleData =
    selector?.getListTaxRuleData?.data?.data?.list || [];
  ...
  useEffect(() => {
    if (selectedTax && subTaxList.length > 0) {
      const filtered = subTaxList.filter(
        (subTax) => (subTax.tax_id?._id || subTax.tax_id) === selectedTax.value,
      );
      ...
      if (selectedSubTax.length > 0) {
        const currentSubTax = subTaxList.find(
          (sub) => sub._id === selectedSubTax[0]?.value,
        );
        const currentSubTaxTaxId =
          currentSubTax?.tax_id?._id || currentSubTax?.tax_id;
  ...
  const tableRows = getListTaxRuleData?.map((rule) => [
    ...
    <div className="flex flex-col" key={`status-${rule._id}`}>
      <ToggleButton
        isToggle={isRowActive(rule)}
        handleClick={() => handleToggle(rule)}
      />
    </div>,
    <span key={`actions-${rule._id}`}>
      <ActionButtons
        onEdit={() => {
          ...
          const ruleSubTaxes = subTaxList.filter(
            (subTax) =>
              rule.subTaxes_id &&
          ...
          setForm({
            ...
            tax_id:
              rule.taxId?._id || rule.tax_id?._id || rule.taxId || rule.tax_id,
            ...
            category_id:
              rule.category_id?._id || rule.category_id || rule.category || "",
          ...
          setSelectedTax(
            selectedTaxValue
              ? {
                  value: selectedTaxValue._id || selectedTaxValue,
                  label: selectedTaxValue.name || "Selected Tax",
                }
              : null,
          );
          ...
          setSelectedCategory(
            selectedCategoryValue
              ? {
                  value: selectedCategoryValue._id || selectedCategoryValue,
                  label: selectedCategoryValue.name || selectedCategoryValue,
                }
              : null,
          );
          setIsEditModal(true);
  ...
  const confirmDelete = () => {
    dispatch(softDeleteTaxRule(deleteData))
      .unwrap()
      .then((res) => {
        if (res.error) {
          toast.error(res.error);
  ...
  const handleSelectAllChange = (e) => {
    if (e.target.checked) {
      const allIds =
        selector?.getListTaxRuleData?.data?.data?.list?.map(
          (rule) => rule._id,
        ) || [];
  ...
  const isAllRowsSelected = useMemo(
    () =>
      selectedRow.length ===
        selector?.getListTaxRuleData?.data?.data?.list?.length &&
      selector?.getListTaxRuleData?.data?.data?.list?.length > 0,
    [
      selectedRow.length,
      selector?.getListTaxRuleData?.data?.data?.list?.length,
    ],
