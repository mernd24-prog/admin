...
const Tax = () => {
  ...
  const mappedCountries =
    countrySelector?.getAllCountryListData?.data?.data?.list?.map((e) => ({
      value: e?._id,
      label: e?.name,
    }));
  ...
  const tableRows = getListData?.map((tax) => [
    ...
    <div className="flex flex-col">
      <ToggleButton
        isToggle={isRowActive(tax)}
        handleClick={() => handleToggle(tax)}
      />
    </div>,
  ...
  const confirmDelete = () => {
    dispatch(softDeleteTaxList(deleteData))
      .unwrap()
      .then((res) => {
        if (res.error) {
          toast.error(res.error);
  ...
  const isAllRowsSelected = useMemo(
    () =>
      selectedRow.length ===
        selector?.getTaxListData?.data?.data?.list?.length &&
      ...
      <div className="max-w-7xl mx-auto">
        <div className=" overflow-hidden overflow-y-auto py-6">
          <div className="flex justify-between items-center">
            <h3>
              <Link to="/app/home">Home</Link> / <b>Tax</b>
            </h3>
            <AddButton
          ...
          <div className="overflow-y-auto bg-white rounded-lg border border-[#E6E6E6]">
            ...
            <TableData
              ...
              tableHeadings={["Tax Name", "Country", "Status", "Actions"]}
              data={tableRows}
              showSearch={false}
              showFilter={false}
              showSummary={false}
