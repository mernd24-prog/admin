...
const getParentId = (user = {}) => {
  const parent = user.parentAdmin || user.parent || user.createdBy || user.createdByUser || user.ownerAdmin;
  return (
    user.parentAdminId ||
    user.parentId ||
...
const getRoleLabel = (role = '') =>
  String(role || '').toLowerCase() === 'seller-admin' ? 'Seller Admin' : 'Sub-Seller';
...
const getOrderId = (order = {}) =>
  order.order_no || order.orderNo || order.order_id || order.orderId || order.id || '-';

const getOrderAmount = (order = {}) =>
  order.total_amount || order.totalAmount || order.payable_amount || order.payableAmount || 0;

const getOrderDate = (order = {}) =>
  order.created_at || order.createdAt || order.orderDate || order.date;

const getProductName = (product = {}) =>
  product.name || product.productName || product.title || product.product_name || product.sku || '-';

const getProductStatus = (product = {}) =>
  product.status || product.approvalStatus || product.productStatus || (product.isActive === false ? 'inactive' : 'active');

const getProductPrice = (product = {}) =>
  product.price || product.salePrice || product.sellingPrice || product.mrp || product.basePrice || 0;

const getProductCreatorId = (product = {}) => {
  const creator = product.createdBy || product.created_by || product.uploadedBy || product.createdByUser;
...
const validateForm = (form) => {
  ...
  if (!form.allowedModules?.length) errors.allowedModules = 'Select at least one management section';
  return errors;
};
...
const buildHierarchy = (users = [], rootUser = {}) => {
  ...
  nodeMap.forEach((node) => {
    ...
    const parent = parentKey && parentKey !== String(node.id) ? nodeMap.get(parentKey) : null;
    if (parent) {
...
const SellerSubAdminManagement = () => {
  ...
  const visibleSubAdmins = useMemo(() => {
    ...
    return subAdmins.filter((user) =>
      [getUserName(user), user.email, user.phone, user.role, ...(user.allowedModules || []).map(getModuleLabel)]
        .join(' ')
        .toLowerCase()
        .includes(query)
    );
  ...
  useEffect(() => {
    ...
    const currentUserId = storedUser?._id || storedUser?.id || storedUser?.userId;
    if (currentUserId) {
  ...
  useEffect(() => {
    setForm((current) => {
      const allowedModules = current.allowedModules.filter((module) => moduleOptions.includes(module));
      return {
        ...current,
        allowedModules: allowedModules.length ? allowedModules : moduleOptions.slice(0, 2),
      };
    });
  }, [moduleOptions]);
  ...
  const openEdit = (user) => {
    ...
    setForm({
      ...
      email: user.email || '',
      phone: user.phone || '',
      allowedModules: allowedModules.length ? allowedModules : moduleOptions.slice(0, 1),
    });
    setModalOpen(true);
  };
  ...
  const handleSubmit = async (event) => {
    ...
    const allowedModules = form.allowedModules.filter((module) => moduleOptions.includes(module));
    const nextErrors = editTarget ? (allowedModules.length ? {} : { allowedModules: 'Select at least one management section' }) : validateForm({ ...form, allowedModules });
    ...
    try {
      ...
      toast.error(error?.message || `Failed to ${editTarget ? 'update' : 'create'} sub-seller`);
    }
  };
  ...
  const runConfirmAction = async () => {
    ...
    try {
      if (confirmAction.type === 'delete') {
        ...
        toast.success(`Sub-seller ${isActive(confirmAction.user) ? 'deactivated' : 'activated'}`);
      }
      refresh();
    } catch (error) {
  ...
  const rows = pagedSubAdmins.map((user) => {
    ...
    return [
      <span key={`name-${id}`} className="font-medium capitalize">{getUserName(user)}</span>,
      <span key={`email-${id}`} className="text-gray-500">{user.email || '-'}</span>,
      <span key={`phone-${id}`} className="text-gray-500">{user.phone || '-'}</span>,
      ...
      <div key={`mods-${id}`} className="flex flex-wrap gap-1">
        ...
        {modules.length > 3 && <span className="text-xs text-gray-400">+{modules.length - 3} more</span>}
      </div>,
      <span key={`status-${id}`} className={isActive(user) ? 'text-green-600' : 'text-red-500'}>
      ...
      <div className="mx-auto max-w-7xl px-4 py-6">
        ...
        {tab === 'subadmins' && (
          <div className="rounded-lg border border-gray-200 bg-white">
            ...
            <TableData
              ...
              tableHeadings={['Name', 'Email', 'Phone', 'Role', 'Management Sections', 'Status', 'Created', 'Actions']}
              data={rows}
        ...
        {tab === 'hierarchy' && (
          <div className="rounded-lg border border-gray-200 bg-white p-4">
            <div className="mb-4">
              <h2 className="text-base font-semibold text-gray-900">Seller Hierarchy</h2>
              <p className="mt-1 text-xs text-gray-500">
                Shows seller admins and sub-sellers below the current seller account.
              </p>
            </div>
              ...
              {hierarchy.map((node) => (
                ...
                >
                  <div>
                    ...
                    <p className="text-xs text-gray-400">{node.email || 'Current seller account'}</p>
                  </div>
                  <span className="text-xs capitalize text-gray-600">{node.role}</span>
                  <span className={node.active === false ? 'text-xs text-red-500' : 'text-xs text-green-600'}>
        ...
        <div className="space-y-4 p-4">
          ...
          <div className="space-y-1.5">
            ...
            <ModuleSelector
              ...
              onChange={(allowedModules) => setForm((current) => ({ ...current, allowedModules }))}
            />
          ...
          <div className="space-y-5">
            <section className="rounded-lg border border-gray-200 bg-white p-4">
              <div className="mb-3 flex items-center justify-between gap-3">
                <div>
                  <h3 className="text-base font-semibold text-gray-900">{getUserName(viewTarget)}</h3>
                  <p className="text-xs text-gray-400">{viewTarget.email || '-'}</p>
                </div>
                <span className={`rounded-full px-3 py-1 text-xs font-medium ${
                  isActive(viewTarget) ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-600'
                }`}>
              ...
              <div className="grid grid-cols-1 gap-3 text-sm sm:grid-cols-2">
                ...
                <div>
                  ...
                  <p className="text-gray-800">{getRoleLabel(viewTarget.role)}</p>
                </div>
                <div>
                  <p className="text-xs uppercase text-gray-400">Created</p>
                  <p className="text-gray-800">{formatDate(viewTarget.createdAt)}</p>
                </div>
                <div>
              ...
              <div className="mt-4">
                <p className="mb-2 text-xs uppercase text-gray-400">Management Sections</p>
            ...
            <section className="rounded-lg border border-gray-200 bg-white p-4">
              <div className="mb-4 grid grid-cols-2 gap-3 sm:grid-cols-4">
                {[
                  { label: 'Orders', value: viewDashboard.totalOrders || viewOrders.length },
                  { label: 'Sub-Seller Products', value: viewProducts.length },
                  { label: 'Revenue', value: formatCurrency(viewDashboard.gmv || viewDashboard.totalRevenue || 0) },
                  { label: 'Returns', value: viewDashboard.returnedOrders || viewDashboard.returns || 0 },
                ].map((item) => (
            ...
            <section className="rounded-lg border border-gray-200 bg-white p-4">
              <div className="mb-3 flex items-center justify-between">
                <h3 className="text-base font-semibold text-gray-900">Seller Orders</h3>
                <span className="rounded-full bg-gray-100 px-3 py-1 text-xs text-gray-600">{viewOrders.length}</span>
              </div>
              {viewLoading ? (
                ...
                <div className="max-h-80 divide-y divide-gray-100 overflow-y-auto rounded border border-gray-100">
                  {viewOrders.map((order) => (
                    <div key={getOrderId(order)} className="grid grid-cols-1 gap-2 p-3 sm:grid-cols-[1fr,120px,120px,110px]">
                      <div>
                        <p className="text-sm font-medium text-gray-900">{getOrderId(order)}</p>
                        <p className="text-xs text-gray-400">{formatDate(getOrderDate(order))}</p>
                      </div>
                      <span className="text-sm text-gray-700">{formatCurrency(getOrderAmount(order), order.currency || 'INR')}</span>
                      <span className="text-xs capitalize text-gray-500">{order.payment_status || order.paymentStatus || '-'}</span>
            ...
            <section className="rounded-lg border border-gray-200 bg-white p-4">
              <div className="mb-3 flex items-center justify-between">
                <h3 className="text-base font-semibold text-gray-900">Sub-Seller Products</h3>
                <span className="rounded-full bg-gray-100 px-3 py-1 text-xs text-gray-600">{viewProducts.length}</span>
              </div>
              {viewLoading ? (
                ...
                <div className="max-h-80 divide-y divide-gray-100 overflow-y-auto rounded border border-gray-100">
                  {viewProducts.map((product) => (
                    <div key={product._id || product.id || product.sku || getProductName(product)} className="grid grid-cols-1 gap-2 p-3 sm:grid-cols-[1fr,110px,100px,110px]">
                      <div>
                        <p className="text-sm font-medium text-gray-900">{getProductName(product)}</p>
                        <p className="text-xs text-gray-400">{product.sku || product.productCode || product._id || product.id || '-'}</p>
                      </div>
                      <span className="text-sm text-gray-700">{formatCurrency(getProductPrice(product), product.currency || 'INR')}</span>
                      <span className="text-xs text-gray-500">Stock: {product.stock ?? product.quantity ?? product.inventory?.quantity ?? '-'}</span>
                      <span className="text-xs capitalize text-gray-600">{getProductStatus(product)}</span>
                    </div>
                  ))}
                </div>
              ) : (
