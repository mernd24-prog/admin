...
const HsnCode = () => {
    ...
    const fetchHsnList = useCallback(async (searchKeyword = filters?.search) => {
        const query = {
            page: pageNo,
            size: PAGE_SIZE,
            select: 'code IGST CGST SGST additionalTax description isDisable createdAt',
    ...
    const handleSubmit = useCallback(async (e) => {
        ...
        try {
            if (isEditMode) {
                await dispatch(updateHsn({ ...basePayload, _id: formData._id })).unwrap()
    ...
    const tableHeadings = ["HSN Code", "IGST (%)", "CGST (%)", "SGST (%)", "Additional Tax", "Description", "Status", "Action"]
    ...
    const tableRows = useMemo(() =>
        apiRes?.list?.map((item) => [
            ...
            <span key={`code-${item._id}`} className="text-sm font-medium text-gray-800 font-mono">
                {item?.code}
            </span>,
            ...
            <span key={`description-${item._id}`} className="text-sm text-gray-700 max-w-xs truncate" title={item?.description}>
                {item?.description || '-'}
            </span>,
            <div key={`toggle-${item._id}`} className={isLoading ? ' cursor-progress' : 'cursor-default'}>
                <ToggleButton
