...
import { create, enableDisable, getList, softDelete, update } from '../../../Redux/patternSlice'
...
const Pattern = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (isEditMode) {
                await dispatch(update({ ...payload, _id: formData._id })).unwrap();
    ...
    const handleAction = async (data) => {
        ...
        try {
            const res = await dispatch(softDelete(apiPayload)).unwrap();
            if (res) {
                toast.success(res?.message)
            }
            fetchCollectionsList();
    ...
    return (
        <>
            <div className='p-6 overflow-hidden max-w-7xl mx-auto overflow-x-auto overflow-y-auto space-y-3'>
                ...
                <DefaultModal
                    title={isEditMode ? 'Edit Pattern' : 'Add Pattern'}
                    isOpen={isAddModal}
                    onClose={closeModal}
