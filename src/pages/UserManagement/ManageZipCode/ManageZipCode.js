...
const ZipCodeFormModal = ({ open, onClose, onSubmit, isEditMode, formData, handleInputChange, handleSelectChange, errors, allCountries, filteredStates, filteredCities }) => {
  if (!open) return null;
  return (
    <div className="fixed inset-0 z-[9998] flex items-center justify-center">
      ...
      <div className="relative w-full max-w-2xl max-h-[92vh] overflow-y-auto bg-white rounded-lg shadow-xl mx-4">
        <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
          <h2 className="text-base font-semibold text-gray-800">{isEditMode ? "Edit Zip Code" : "Add Zip Code"}</h2>
          <button type="button" onClick={onClose} className="text-gray-400 hover:text-gray-700">✕</button>
        </div>
        <form onSubmit={onSubmit} className="p-5">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <Input labelName="Zip / Pin Code" type="text" value={formData.zipCode} name="zipCode" onChange={handleInputChange} error={errors.zipCode} required maxLength={10} />
            <Input labelName="Area Name" type="text" value={formData.areaName} name="areaName" onChange={handleInputChange} error={errors.areaName} maxLength={100} />

            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Country <span className="text-red-500">*</span></label>
              <FilterSelect options={allCountries} value={allCountries.find((o) => o.value === formData.countryId) || null} onChange={handleSelectChange} name="countryId" isSearchable placeholder="Select Country" />
              {errors.countryId && <p className="mt-1 text-sm text-red-600">{errors.countryId}</p>}
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">State <span className="text-red-500">*</span></label>
              <FilterSelect options={filteredStates} value={filteredStates.find((o) => o.value === formData.stateId) || null} onChange={handleSelectChange} name="stateId" isSearchable placeholder="Select State" isDisabled={!formData.countryId} />
              {errors.stateId && <p className="mt-1 text-sm text-red-600">{errors.stateId}</p>}
            </div>
            <div className="col-span-2">
              <label className="block text-sm font-medium text-gray-700 mb-1">City <span className="text-red-500">*</span></label>
              <FilterSelect options={filteredCities} value={filteredCities.find((o) => o.value === formData.cityId) || null} onChange={handleSelectChange} name="cityId" isSearchable placeholder="Select City" isDisabled={!formData.stateId} />
              {errors.cityId && <p className="mt-1 text-sm text-red-600">{errors.cityId}</p>}
            </div>

            <Input labelName="Delivery Charge (₹)" type="number" value={formData.deliveryCharge} name="deliveryCharge" onChange={handleInputChange} error={errors.deliveryCharge} min={0} />
            <Input labelName="Min Order Amount (₹)" type="number" value={formData.minOrderAmount} name="minOrderAmount" onChange={handleInputChange} error={errors.minOrderAmount} min={0} />
            <Input labelName="Est. Delivery Days" type="number" value={formData.estimatedDeliveryDays} name="estimatedDeliveryDays" onChange={handleInputChange} error={errors.estimatedDeliveryDays} min={1} />

            <div className="col-span-2 flex flex-wrap gap-6 pt-2">
              {[["serviceable", "Serviceable"], ["codAvailable", "COD Available"], ["expressDelivery", "Express Delivery"]].map(([name, label]) => (
                <label key={name} className="flex items-center gap-2 cursor-pointer">
                  <input type="checkbox" name={name} checked={!!formData[name]} onChange={handleInputChange} className="w-4 h-4 accent-primary" />
          ...
          <div className="flex items-center justify-end gap-3 pt-4 border-t border-gray-100 mt-4">
            ...
            <button type="submit" className="admin-btn-primary">{isEditMode ? "Update" : "Add Zip Code"}</button>
          </div>
        </form>
      </div>
    </div>
...
const ManageZipCode = () => {
  ...
  const allCountries =
    selector?.country?.getAllCountryListData?.data?.data?.list?.map((e) => ({ value: e?._id, label: e?.name })) || [];
  ...
  useEffect(() => {
    if (formData.countryId) {
      setFilteredStates(
        allStates.filter((s) => s.countryId === formData.countryId || s.countryId?._id === formData.countryId)
  ...
  useEffect(() => {
    if (formData.stateId) {
      dispatch(getAllCityList({ stateId: formData.stateId })).then((res) => {
        setFilteredCities(extractListPayload(res?.payload).list.map((c) => ({ value: c._id, label: c.name })));
      });
    } else {
      setFilteredCities([]);
    }
  ...
  const isRowActive = (row = {}) => row?.active !== undefined ? Boolean(row.active) : !row?.isDisable;

  const handleToggleConfirm = async () => {
    if (!toggleTarget) return;
    try {
      const res = await dispatch(enableDisableZipCode({ _id: [toggleTarget?._id], isDisable: isRowActive(toggleTarget) })).unwrap();
  ...
  const columns = [
    ...
    },
    {
      ...
      label: "Status",
      render: (_, row) => (
        <ToggleButton isToggle={isRowActive(row)} handleClick={() => setToggleTarget(row)} />
      ),
    },
  ];
