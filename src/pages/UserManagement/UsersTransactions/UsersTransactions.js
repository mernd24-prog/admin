...
import { createContentPage, getContentPages } from '../../../Redux/adminCoreSlice';
...
const UsersTransactions = () => {
  ...
  const tableRows = apiRes?.map((item) => {
    ...
    return [
      ...
      <span className='p-1 bg-sky-100 text-sky-600' key={`status-${item?._id || item?.slug}`}>
  ...
  const handleCreateTransaction = async (data) => {
    try {
      const selectedUser = userOptions.find((u) => `${u.name} (${u.email})` === data.userId);
      const now = Date.now();
      const txId = `TN-${String(now).slice(-8)}`;

      await dispatch(createContentPage({
        ...
        metadata: {
          ...
          userLabel: selectedUser ? `${selectedUser.name} (${selectedUser.email})` : data.userId,
