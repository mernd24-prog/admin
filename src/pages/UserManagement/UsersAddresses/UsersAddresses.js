...
import { create,enableDisableUser,getUserAddressList,update, updatePasswordUser } from '../../../Redux/userManagementSlice';
...
const UsersAddresses = () => {
  ...
  useEffect(() => {
    const reqData = {
      ...
      searchFields: 'house_flat_floor',
      sortOrder:'desc',
      sortBy:'createdAt',
      populate:'city_code:name|user_id:email',
      select: 'createdAt email user_id house_flat_floor isDisable'
    };
    dispatch(getUserAddressList(reqData));
  }, [size, pageNo, dispatch, isRefresh]);

  const selector = useSelector(state => state.user);
  const getListData = selector?.getUserAddressListData?.data?.data;
  ...
  const tableRows = getListData?.list?.map((user) => [
    ...
    <span key={`email-${user._id}`}>
      ...
      <span className='text-gray-500'>{user?.user_id?.email || "N/A"}</span>
    </span>,
    <span key={`phone-${user._id}`}>
      <span className='capitalize'>{user?.house_flat_floor || "Not Available"}</span>
    </span>,
     <span key={`phone-${user._id}`}>
      <span className='capitalize'>{user?.city_code?.name || "Not Available"}</span>
    </span>,
     <span key={`phone-${user._id}`}>
  ...
  const handleApplySearchFilters = () => {
    const reqData = {
      ...
      searchFields: 'house_flat_floor',
      populate:'city_code:name|user_id:email',
      select: 'createdAt email user_id isDisable house_flat_floor'
      ...
      <div className='max-w-7xl mx-auto'>
        <div className=' overflow-hidden overflow-y-auto py-6'>
          <div className="flex justify-between items-center">
           <h3><Link to="/app/home"className='cursor-pointer'>Home</Link> / <b>User Addresses</b></h3>
          </div>
