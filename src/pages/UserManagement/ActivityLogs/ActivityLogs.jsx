...
const COLUMNS = [
  ...
  },
  {
    ...
    render: (_, row) => (
      <div>
        <div className="font-medium text-gray-700 text-xs">{row.actorName || '—'}</div>
  ...
  },
  {
    ...
    label: 'Action',
    render: (v) => (
      <span className={`text-[10px] px-2 py-0.5 rounded-full font-semibold uppercase tracking-wide ${ACTION_BADGE[v?.toLowerCase()] || 'bg-gray-100 text-gray-500'}`}>
        {v || '—'}
      </span>
    ),
  },
  { key: 'module',      label: 'Module',    render: (v) => <span className="text-xs text-gray-600 capitalize">{v || '—'}</span> },
  { key: 'description', label: 'Description', render: (v) => <span className="text-xs text-gray-500 line-clamp-2">{v || '—'}</span> },
  {
    key: 'ip',
    label: 'IP',
...
const ACTION_FILTERS = ['all', 'create', 'update', 'delete', 'login', 'logout', 'approve', 'reject'];
