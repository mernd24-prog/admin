...
const MODULE_ROUTE_EXPANSIONS = {
  ...
  orders: [
    ...
    { label: "Subscription Orders", route: "subscription-orders" },
    {
      ...
      route: "order-cancellation-reasons",
    },
  ...
  products: [
    ...
    { label: "Product Attributes", route: "product-options" },
  ...
  pricing: [
    ...
    {
      label: "Recommended Tag Weightages",
      route: "recommended-product-tag-weightages",
    },
  ],
};
...
const Sidebar = ({
  ...
  const sidebarData = useMemo(() => {
    if (sellerPanel) {
      ...
      return Object.entries(groupedByTab)
        ...
        .map(([tabName, modules]) => ({
          ...
          subItems: modules,
          isSingleItem:
            tabName.toLowerCase() === "home" && modules.length === 1,
        }));
    }
    ...
    const groupedByTab = permissions?.reduce((acc, curr) => {
      const hasAssignedPermission = (curr.permissions || []).some(
        (p) => p.assigned,
      );
          ...
          <ul>
            {sidebarData.map((item, index) => {
              ...
              const hasActiveChild = item.subItems.some(
                (subItem) =>
                  location.pathname === `/app/${subItem.module_code}`,
              );
                  ...
                  >
                    ...
                    >
                      <IconComponent
                        ...
                        className="bg-white text-[#989AFF]"
                      />
                      {isExpanded && (
              ...
              return (
                ...
                >
                  ...
                  >
                    <IconComponent
                      size={isExpanded ? 22 : 18}
                      className="bg-white text-[#CE9F2D]"
                    />
                    {isExpanded && (
                      <>
                        <span className="text-xs">{item.label}</span>
                      ...
                      <ul className="mt-1 ml-6 space-y-1">
                        {item?.subItems?.map((subItem, subIndex) => {
                          ...
                          return (
                            ...
                            >
                              <Link
                                className={`flex items-center gap-3 p-2 text-sm transition-all duration-200 ease-in-out rounded ${
                                  isSubActive
                                ...
                                to={`/app/${subItem.module_code}`}
                                onClick={() =>
                                  handleNavClick(subItem.module_code)
                                }
                              >
