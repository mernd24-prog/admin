  ...
  const normalizedAvailable = Array.from(
    new Set(
      ["none", ...(availablePermissions || [])]
        .map(normalizeAction)
        .filter(
          (permission) => permission && ACTION_ORDER.includes(permission),
        ),
    ),
  );
  ...
  const handlePermissionChange = (optionValue) => {
    if (optionValue === "none") {
      ...
      if (selectedSet.has(optionValue)) {
        ...
        newSelected = [
          ...normalizedSelected.filter((perm) => perm !== "none"),
          optionValue,
        ];
      }
        ...
        .map((option) => {
          ...
          return (
            ...
            >
              ...
              />
              <span
                className={`${isSelected ? "font-medium text-gray-800" : "font-normal text-gray-600"}`}
              >
