...
export default function SearchComponent({
  ...
  return (
    <div className="w-full bg-white p-1 ">
      <div className={`flex md:flex-row flex-col justify-between gap-10 items-center mb-4 ${mobailClassName}`}>
          ...
          <div className={` transition-all duration-300 ease-in-out ${searchDown ? 'opacity-100 mb-4 ' : 'opacity-0 max-h-0'}`}   >
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 items-end text-xs">
              ...
              {isUser && (
                <div>
                  <FilterSelect
                    ...
                    options={userOptions}
                    onChange={(option) => handleFilterChange('sellerName', option)}
                  />
                </div>
              )}
              {isDelete && (
              ...
              {isActivationStatus && (
                <div>
                  <FilterSelect
                    label={activationStatus ? activationStatus : `Activation status`}
                    value={filters.activationStatus}
                    options={activationStatusOptions}
                    onChange={(option) => handleFilterChange('activationStatus', option)}
                  />
                </div>
              )}
              ...
              {isApprovalOptions && (
                <div>
                  <FilterSelect
                    ...
                    onChange={(option) => handleFilterChange('approvalStatus', option)}
                  />
                </div>
              )}
              ...
              {isProductType && (
                <div>
                  <FilterSelect
                    ...
                    onChange={(option) => handleFilterChange('productType', option)}
                  />
                </div>
              )}
              ...
              {
                orderFrom && (
                  <Input label={`Order From`} labelName={fromLabel ? fromLabel : 'Order From'} placeholder={`Order from [$]`} />
                )
              }
              {
                orderTo && (
                  <Input label={`Order To`} labelName={toLabel ? toLabel : "Order To"} placeholder={`Order to [$]`} />
                )
              }
