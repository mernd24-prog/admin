...
const SubscriptionPanel = ({ subscription = {}, onChange }) => {
  ...
  const addFeature = () => {
    ...
    const existing = Array.isArray(subscription.features)
  ...
  const removeFeature = (f) => {
    set(
      "features",
      (subscription.features || []).filter((x) => x !== f),
    );
  };
    ...
    <div className="space-y-5">
      <div className="bg-purple-50 border border-purple-200 rounded-lg p-3">
        ...
        <p className="text-xs text-purple-600 mt-0.5">
          Customers are billed on a recurring basis. Configure the billing
          cycle, trial, and pricing below.
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <Field
          label="Setup Fee (₹)"
          hint="One-time charge at sign-up. 0 = no setup fee."
        >
          <input
            type="number"
            min={0}
      ...
      <Field
        ...
        hint="Extra days after a failed payment before cancelling. Default 3 days."
      >
        <input
          type="number"
          min={0}
      ...
      <Field
        label="Plan Features"
        hint="List what's included in this plan (e.g. 'Unlimited storage', '24/7 support')."
      >
          ...
          {(subscription.features || []).length > 0 && (
            <ul className="space-y-1">
              {(subscription.features || []).map((f, i) => (
                <li
                  key={i}
                  className="flex items-center justify-between px-3 py-1.5 bg-gray-50 border border-gray-200 rounded-md text-sm text-gray-700"
                >
        ...
        {[
          ...
          >
            <input
              ...
              checked={
                subscription[key] !== undefined ? subscription[key] : defaultVal
              }
