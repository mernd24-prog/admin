  ...
  const doSearch = useCallback(async () => {
    ...
    try {
      const res = await dispatch(
        getProducts({ q: search.trim(), limit: 10, status: "approved" }),
      ).unwrap();
      const docs = res?.data?.products || res?.data?.docs || [];
      setResults(
        docs.filter(
          (p) => !bundleItems.some((bi) => bi.productId === (p._id || p.id)),
        ),
      );
    } catch {
      setResults([]);
    ...
    onChange(
      bundleItems.map((bi) =>
        bi.productId === productId
          ? { ...bi, quantity: Math.max(1, Number(qty)) }
          : bi,
      ),
    );
  };

  const componentTotal = bundleItems.reduce(
    ...
    <div className="space-y-5">
      <div className="bg-orange-50 border border-orange-200 rounded-lg p-3">
        ...
        <p className="text-xs text-orange-600 mt-0.5">
          Search and add products to this bundle. Set quantities and a combo
          discount.
        </p>
      </div>
            ...
            <div className="absolute z-10 w-full bg-white border border-gray-200 rounded-md shadow-lg mt-1 max-h-52 overflow-y-auto">
              {results.map((p) => (
                ...
                >
                  ...
                  <span className="ml-auto text-xs text-[var(--admin-blue)] font-medium flex-shrink-0">
                    + Add
                  </span>
                </button>
              ))}
      ...
      {bundleItems.length > 0 ? (
        <div className="space-y-2">
          ...
          <div className="mt-4 p-4 bg-white border border-gray-200 rounded-lg space-y-3">
            ...
            <div className="flex items-center justify-between pt-2 border-t border-gray-100">
              ...
              <span className="text-base font-bold text-[var(--admin-blue)]">
                ₹
            ...
            {bundleDiscount > 0 && (
              <p className="text-xs text-green-600">
                Customers save {bundleDiscount}% vs buying separately
              </p>
            )}
          </div>
        </div>
        ...
        <div className="flex flex-col items-center justify-center py-8 border-2 border-dashed border-gray-200 rounded-lg text-gray-400">
          ...
          <p className="text-sm">
            Search and add products to build your bundle
          </p>
        </div>
      )}
    </div>
